...
```moo
(#2): ;;for obj in (connected_players())
        if (is_wizard(obj))
          notify(obj.name + " (" + tostr(obj) + ")");
        endif
      endfor
```
...
```moo
(#2): ;;for p in (connected_players())
        p.location:announce(p.name + " has disconnected.");
      endfor
```
...
- Uses `SYSTEM_OBJECT` permissions for admin-level access
