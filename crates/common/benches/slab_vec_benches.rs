...
use moor_common::util::{SlabVec, TaskVecPool, VecPool};
use std::rc::Rc;
...
impl BenchContext for SmallSlabVecContext {
    fn prepare(_num_chunks: usize) -> Self {
        ...
        let vec = SlabVec::new(pool.clone());
        Self { pool, vec }
    }
}
...
impl BenchContext for MediumSlabVecContext {
    fn prepare(_num_chunks: usize) -> Self {
        let pool = Rc::new(RefCell::new(VecPool::new()));
        let vec = SlabVec::with_capacity(pool.clone(), 32);
        Self { pool, vec }
    }
}
...
impl BenchContext for LargeSlabVecContext {
    fn prepare(_num_chunks: usize) -> Self {
        let pool = Rc::new(RefCell::new(VecPool::new()));
        let vec = SlabVec::with_capacity(pool.clone(), 128);
        Self { pool, vec }
    }
}
...
impl BenchContext for PoolReuseContext {
    fn prepare(_num_chunks: usize) -> Self {
        ...
        for _ in 0..10 {
            let mut vec = SlabVec::with_capacity(pool.clone(), 64);
            for i in 0..32 {
                vec.push(i);
            }
            vecs.push(vec);
...
fn slabvec_push_small(ctx: &mut SmallSlabVecContext, chunk_size: usize, _chunk_num: usize) {
    let mut vec = SlabVec::new(ctx.pool.clone());
    for i in 0..chunk_size.min(16) { // Respect size class limit
        vec.push(i as i64);
        black_box(&vec);
    }
}
...
fn slabvec_push_medium(ctx: &mut MediumSlabVecContext, chunk_size: usize, _chunk_num: usize) {
    let mut vec = SlabVec::with_capacity(ctx.pool.clone(), 32);
...
fn slabvec_push_large(ctx: &mut LargeSlabVecContext, chunk_size: usize, _chunk_num: usize) {
    let mut vec = SlabVec::with_capacity(ctx.pool.clone(), 128);
...
fn slabvec_pop_small(ctx: &mut SmallSlabVecContext, chunk_size: usize, _chunk_num: usize) {
    // Pre-fill vector
    let mut vec = SlabVec::with_capacity(ctx.pool.clone(), 16);
    for i in 0..16 {
        vec.push(i);
    }
...
fn pool_reuse_allocation(ctx: &mut PoolReuseContext, chunk_size: usize, _chunk_num: usize) {
    ...
    for i in 0..chunk_size {
        let mut vec = SlabVec::with_capacity(ctx.pool.clone(), 64);
        for j in 0..(i % 32 + 1) {
            vec.push(j as i64);
        }
...
fn taskvecpool_create_destroy(ctx: &mut TaskVecPoolContext, chunk_size: usize, _chunk_num: usize) {
    for i in 0..chunk_size {
        let mut vec = SlabVec::with_capacity(ctx.pool.inner().clone(), 64);
        for j in 0..(i % 32 + 1) {
            vec.push(j as i64);
        }
        black_box(vec);
...
pub fn main() {
    ...
    if let Some(f) = filter {
        ...
        eprintln!("Available filters: all, small, medium, large, pool, comparison, or any benchmark name substring");
        eprintln!();
    }

    // SlabVec operation benchmarks
    let small_slabvec_benchmarks = [
        BenchmarkDef { name: "slabvec_push_small", group: "small", func: slabvec_push_small, throughput_elements: None },
        BenchmarkDef { name: "slabvec_pop_small", group: "small", func: slabvec_pop_small, throughput_elements: None },
        BenchmarkDef { name: "slabvec_clone_small", group: "small", func: slabvec_clone_small, throughput_elements: None },
    ];

    let medium_slabvec_benchmarks = [
        BenchmarkDef { name: "slabvec_push_medium", group: "medium", func: slabvec_push_medium, throughput_elements: None },
        BenchmarkDef { name: "slabvec_resize_grow", group: "medium", func: slabvec_resize_grow, throughput_elements: None },
    ];

    let large_slabvec_benchmarks = [
        BenchmarkDef { name: "slabvec_push_large", group: "large", func: slabvec_push_large, throughput_elements: None },
        BenchmarkDef { name: "slabvec_clone_large", group: "large", func: slabvec_clone_large, throughput_elements: None },
    ];

    let pool_benchmarks = [
        BenchmarkDef { name: "pool_reuse_allocation", group: "pool", func: pool_reuse_allocation, throughput_elements: None },
    ];

    // Standard Vec comparison benchmarks
    let comparison_benchmarks = [
        BenchmarkDef { name: "stdvec_push_small", group: "comparison", func: stdvec_push_small, throughput_elements: None },
        BenchmarkDef { name: "stdvec_push_medium", group: "comparison", func: stdvec_push_medium, throughput_elements: None },
        BenchmarkDef { name: "stdvec_push_large", group: "comparison", func: stdvec_push_large, throughput_elements: None },
        BenchmarkDef { name: "stdvec_clone_small", group: "comparison", func: stdvec_clone_small, throughput_elements: None },
        BenchmarkDef { name: "stdvec_clone_large", group: "comparison", func: stdvec_clone_large, throughput_elements: None },
        BenchmarkDef { name: "stdvec_rapid_alloc", group: "comparison", func: stdvec_rapid_alloc, throughput_elements: None },
    ];

    // Run benchmark groups
    run_benchmark_group::<SmallSlabVecContext>(&small_slabvec_benchmarks, "SlabVec Small (16 elements)", filter);
    run_benchmark_group::<MediumSlabVecContext>(&medium_slabvec_benchmarks, "SlabVec Medium (32 elements)", filter);
    run_benchmark_group::<LargeSlabVecContext>(&large_slabvec_benchmarks, "SlabVec Large (128 elements)", filter);
    ...
    let taskvec_pool_benchmarks = [
        BenchmarkDef { name: "taskvecpool_create_destroy", group: "pool", func: taskvecpool_create_destroy, throughput_elements: None },
    ];
    run_benchmark_group::<TaskVecPoolContext>(&taskvec_pool_benchmarks, "TaskVecPool Operations", filter);
