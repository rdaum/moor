...
pub fn complex_match_objects_keys(token: &str, objects: &[Var], keys: &[Var]) -> ComplexMatchResult<Var> {
    let (ordinal, subject) = parse_input_token(token);
...
mod tests {
    use super::*;
    use moor_var::{v_str, v_list, v_int};
    ...
    fn test_parse_input_token() {
        ...
        assert_eq!(parse_input_token("twenty-first lamp"), (21, "lamp".to_string()));
    }
