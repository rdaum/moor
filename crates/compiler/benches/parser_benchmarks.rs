...
use moor_compiler::{
    compile_with_tree_sitter, parse_program_with_tree_sitter, parse_with_semantic_walker,
};
...
fn benchmark_parsers(c: &mut Criterion) {
    let test_cases = vec![
        ...
        (
            "If Statement",
            r#"if (x > 0) notify(player, "positive"); else notify(player, "non-positive"); endif"#,
        ),
        ("For Loop", "for i in [1..10] total = total + i; endfor"),
        (
            "Function Call",
            r#"result = $string_utils:capitalize($object_utils:name(player));"#,
        ),
        (
            "Try-Except",
            r#"try result = 1/0; except (e) notify(player, "Error: " + tostr(e)); endtry"#,
        ),
        (
            "Scatter Assignment",
            r#"let {things, ?nothingstr = "nothing", @rest} = args;"#,
        ),
        (
            "Complex Expression",
            r#"return $object_utils:ancestors(this)[1]:(verbs()[random(length(verbs()))])(player, player.location);"#,
        ),
        (
            "Large Program",
            r#"
    ...
    for (test_name, test_code) in test_cases.iter() {
        ...
        group.bench_with_input(
            ...
            test_code,
            |b, code| b.iter(|| black_box(compile(black_box(code), CompileOptions::default()))),
        );
        ...
        #[cfg(feature = "tree-sitter-parser")]
        {
            ...
            group.bench_with_input(
                ...
                |b, code| {
                    b.iter(|| {
                        black_box(parse_program_with_tree_sitter(
                            black_box(code),
                            CompileOptions::default(),
                        ))
                    })
            ...
            group.bench_with_input(
                ...
                |b, code| {
                    b.iter(|| {
                        black_box(compile_with_tree_sitter(
                            black_box(code),
                            CompileOptions::default(),
                        ))
                    })
            ...
            group.bench_with_input(
                ...
                test_code,
                |b, code| b.iter(|| black_box(parse_with_semantic_walker(black_box(code)))),
            );
        }
    }
...
fn benchmark_scatter_assignment_specifically(c: &mut Criterion) {
    let scatter_cases = vec![
        ...
        (
            "Complex Scatter",
            r#"let {things, ?nothingstr = "nothing", @rest} = $object_utils:inventory(player);"#,
        ),
        (
            "Nested Scatter",
            r#"let {{inner_x, inner_y}, z} = {{1, 2}, 3};"#,
        ),
    ];
...
criterion_group!(
    benches,
    benchmark_parsers,
    benchmark_scatter_assignment_specifically
);
