...
use crate::parsers::tree_sitter::tree_traits::{
    ASTBuilder, ASTBuilderContext, NodeHandlerRegistry, TreeNode,
};
...
impl<T: TreeNode> GenericASTBuilder<T> {
    ...
    fn register_language_specific_handlers(&mut self) {
        ...
        self.handlers.register("error_literal", |node, builder| {
            ...
            let errcode_text = errcode_node
                .text()
                .ok_or_else(|| builder.parse_error(errcode_node, "Error code has no text"))?;

            let error_code = moor_var::ErrorCode::parse_str(errcode_text).ok_or_else(|| {
                builder.parse_error(
                    errcode_node,
        ...
        self.handlers.register("method_call", |node, builder| {
            ...
            Ok(Expr::Verb {
                location: Box::new(
                    object.unwrap_or(Expr::Value(moor_var::v_obj(moor_var::SYSTEM_OBJECT))),
                ),
    ...
    fn extract_map_pairs(&mut self, node: &T) -> Result<Vec<(Expr, Expr)>, CompileError> {
        ...
        for child in node.content_children() {
            if child.node_kind() == "map_entry" {
                ...
                let value = child.child_by_name("value").ok_or_else(|| {
                    ASTBuilder::parse_error(self, child, "Map entry missing value")
                })?;
    ...
    fn default_expression_handler(&mut self, node: &T) -> Result<Expr, CompileError> {
        ...
        if let Some(text) = node.text() {
            if text.chars().all(|c| c.is_ascii_digit()) {
                ...
                let value = text.parse::<i64>().map_err(|e| {
                    ASTBuilder::parse_error(self, node, &format!("Invalid integer: {}", e))
                })?;
            ...
            if text.starts_with('"') && text.ends_with('"') {
                ...
                let parsed = unquote_str(text).map_err(|e| {
                    ASTBuilder::parse_error(self, node, &format!("Invalid string: {}", e))
                })?;
        ...
        Err(ASTBuilder::parse_error(
            self,
            node,
            &format!("Unknown node type: {}", node.node_kind()),
        ))
    }
}
...
impl<T: TreeNode> ASTBuilder<T> for GenericASTBuilder<T> {
    fn build_ast(&mut self, root: &T) -> Result<ParseCst, CompileError> {
        ...
        let statements = match root.node_kind() {
            ...
            _ => {
                return Err(ASTBuilder::parse_error(
                    self,
                    root,
    ...
    where
        F: Fn(&T, &mut dyn ASTBuilderContext<T>) -> Result<Expr, CompileError>
            + Send
            + Sync
            + 'static,
    {
...
mod tests {
    ...
    fn test_generic_ast_builder_creation() {
        ...
        let _builder = super::GenericASTBuilder::<MockNode>::new(options);
    ...
    fn test_build_simple_expression() {
        let options = CompileOptions::default();
        let mut builder = super::GenericASTBuilder::<MockNode>::new(options);
