...
//! Simplified generic AST builder implementation
//! 
//! This provides a working demonstration of the generic tree interface concept
//! without the complexity of the full handler registry system.
...
/// Simplified generic AST builder that works with any TreeNode implementation
/// 
/// This demonstrates the concept of a generic tree interface by providing
/// a basic AST builder that can work with different tree representations.
#[cfg_attr(not(test), allow(dead_code))]
pub struct SimpleGenericBuilder<T: TreeNode> {
...
impl<T: TreeNode> SimpleGenericBuilder<T> {
    /// Create a new simple generic AST builder
    ...
    pub fn build_ast(&mut self, root: &T) -> Result<ParseCst, CompileError> {
        let statements = match root.node_kind() {
            ...
            _ => {
                return Err(self.parse_error(root, &format!("Expected program node, got {}", root.node_kind())));
            }
        };
        ...
        match node.node_kind() {
            ...
            "identifier" => {
                ...
                let name = self.names.borrow_mut()
                    ...
                    .unwrap_or_else(|| {
                        ...
                        new_scope.find_or_add_name_global("error", DeclType::Unknown).unwrap()
                    });
                Ok(Expr::Id(name))
            }
            "integer_literal" => {
            ...
            "expression" | "atom" => {
                ...
                if children.len() == 1 {
                    ...
                    Err(self.parse_error(node, &format!("Unexpected expression structure with {} children", children.len())))
                }
            }
        ...
        match node.node_kind() {
            "expression_statement" => {
                let expr = if let Some(expr_node) = node.child_by_name("expression") {
                    ...
                    let children: Vec<_> = node.content_children().collect();
                    if children.is_empty() {
                        return Err(self.parse_error(node, "Expression statement has no expression"));
                    }
                    self.build_expression(children[0])?
                };
...
pub fn parse_with_generic_builder<T: TreeNode>(
    root: &T,
    options: CompileOptions,
) -> Result<ParseCst, CompileError> {
    let mut builder = SimpleGenericBuilder::new(options);
...
mod tests {
    ...
    #[test]
    fn test_simple_generic_builder() {
        let options = CompileOptions::default();
        let mut builder = SimpleGenericBuilder::<MockNode>::new(options);
    ...
    fn test_parse_with_generic_builder() {
        ...
        let result = parse_with_generic_builder(&program, options);
        assert!(result.is_ok());
    }
}