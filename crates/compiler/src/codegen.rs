...
use crate::parse2::{Parse2, parse_program2};
...
fn do_compile2(parse2: Parse2, compile_options: CompileOptions) -> Result<Program, CompileError> {
    // Generate the code into 'cg_state' - same as do_compile but with Parse2
    let mut cg_state = CodegenState::new(compile_options, parse2.names);
    for x in parse2.stmts {
        cg_state.generate_stmt(&x)?;
    }
    cg_state.emit(Op::Done);
...
pub fn compile(program: &str, options: CompileOptions) -> Result<Program, CompileError> {
    let parse2 = parse_program2(program, options.clone())?;

    do_compile2(parse2, options)
}
    ...
    fn test_cst_vs_legacy_bytecode_parity() {
        ...
        for source in test_cases {
            ...
            assert_eq!(
                ...
                "Variable names mismatch for: {}", source
            );
