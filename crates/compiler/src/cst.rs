...
impl CSTNode {
    ...
    pub fn semantic(rule: Rule, fields: std::collections::HashMap<String, CSTNode>, span: CSTSpan) -> Self {
        Self {
            rule,
            span,
...
impl PestToCSTConverter {
    ...
    pub fn convert_pair(&self, pair: pest::iterators::Pair<Rule>) -> CSTNode {
        ...
        if inner_pairs.is_empty() {
            ...
            let children = self.convert_children_with_whitespace(&inner_pairs, span.start, span.end);
    ...
    pub fn pretty_print(&self, indent: usize) -> String {
        ...
        match &self.kind {
            CSTNodeKind::Terminal { text } => {
                format!("{}Terminal({:?}) [{}] \"{}\"", indent_str, self.rule, span_info, text)
            }
            ...
            CSTNodeKind::NonTerminal { children } => {
                let mut result = format!("{}NonTerminal({:?}) [{}]", indent_str, self.rule, span_info);
                for child in children {
                    result.push('\n');
    ...
    pub fn to_source(&self) -> String {
        match &self.kind {
            ...
            CSTNodeKind::Semantic { fields } => {
                ...
                fields.values().map(|field| field.to_source()).collect::<Vec<_>>().join(" ")
            }
        }
    }
}
...
mod tests {
    ...
    fn test_cst_creation() {
        ...
        let pairs = MooParser::parse(Rule::program, source)
            .expect("Failed to parse");
        
        let converter = PestToCSTConverter::new(source.to_string());
        let cst = converter.convert_program(pairs.into_iter().next().unwrap())
    ...
    fn test_comment_preservation() {
        ...
        let whitespace_nodes: Vec<_> = cst.find_all_nodes()
            .into_iter()
    ...
    I: Fn(Expr, &CSTNode) -> Result<Expr, CompileError>,
{
    ...
    fn parse_expression_with_pos(
        ...
        while pos < nodes.len() {
            ...
            let (right, new_pos) = self.parse_expression_with_pos(nodes, pos, next_min_precedence)?;
            pos = new_pos;
