...
impl CSTComparator {
    ...
    fn compare_nodes(&mut self, pest: &CSTNode, ts: &CSTNode, path: &str) {
        ...
        match (&pest.kind, &ts.kind) {
            (CSTNodeKind::Terminal { text: pest_text }, CSTNodeKind::Terminal { text: ts_text }) => {
            ...
            (CSTNodeKind::NonTerminal { children: pest_children }, CSTNodeKind::NonTerminal { children: ts_children }) => {
...
pub fn cst_to_tree_string(node: &CSTNode, indent: usize) -> String {
    ...
    match &node.kind {
        ...
        CSTNodeKind::NonTerminal { children } => {
            writeln!(&mut output, "{}{:?}: {} children", indent_str, node.rule, children.len()).unwrap();
            for child in children {
                output.push_str(&cst_to_tree_string(child, indent + 1));
            }
        }
        _ => {
            writeln!(&mut output, "{}{:?}: {:?}", indent_str, node.rule, node.kind).unwrap();
        }
    }
...
mod tests {
    ...
    fn test_cst_comparison() {
        ...
        let cst1 = CSTNode {
            rule: Rule::expr,
            span: crate::cst::CSTSpan { start: 0, end: 1, line_col: (1, 1) },
            kind: CSTNodeKind::Terminal { text: "x".to_string() },
        };
