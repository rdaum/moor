    ...
    pub fn expected_tokens(&self) -> Vec<String> {
        match self {
            ...
            ParseContext::WhileLoop => {
                vec!["condition expression".to_string(), "'endwhile'".to_string()]
            }
            ...
            ParseContext::Unknown(_) => vec!["valid syntax".to_string()],
        }
    }
}
...
fn add_source_context(message: &mut String, source: &str, enhanced_error: &EnhancedError) {
    ...
    message.push_str(&format!(
        "\n{:4} | {}",
        enhanced_error.span.start.line, error_line
    ));
    ...
    message.push_str(&format!(
        "\n{:4} | {}{}",
        "", pointer_info.spaces, pointer_info.indicators
    ));
    ...
    if let Some(token_pos) = error_line[start_col..].find(problem_token) {
        ...
        PointerInfo {
            ...
            indicators: format!(
                ...
                if actual_length < problem_token.len() {
                    "..."
                } else {
                    ""
                }
...
pub fn infer_parse_context(source: &str, error_position: &ErrorPosition) -> ParseContext {
    ...
    let before_error = &current_line[..error_position
        .column
        .saturating_sub(1)
...
mod tests {
    ...
    fn test_enhanced_error_message() {
        ...
        let error = EnhancedError::new(span, "@invalid".to_string(), ParseContext::Expression);
