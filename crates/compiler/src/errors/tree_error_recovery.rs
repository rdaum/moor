...
pub enum TreeErrorType {
    ...
    Unclosed { delimiter: String, opened_at: ErrorPosition },
        ...
        match self.error_type() {
            ...
            Some(TreeErrorType::Extra { ref found }) => {
                ...
                match (found.as_str(), context) {
                    ...
                    (":", ParseContext::Unknown(ctx)) if ctx.contains("expression") => {
                        "Incomplete method call - add method name and arguments after ':'".to_string()
                    }
                    (";", _) => "Unexpected semicolon".to_string(),
                    _ => format!("Unexpected {}", found)
                }
            }
            Some(TreeErrorType::Invalid { ref reason }) => {
                reason.clone()
            }
            Some(TreeErrorType::Unclosed { ref delimiter, ref opened_at }) => {
        ...
        match error_node.error_type() {
            Some(TreeErrorType::Missing { ref expected }) => {
                ...
                for exp in expected {
                    let suggestion = match exp.as_str() {
                        ";" => ErrorFix::insertion(pos.clone(), ";".to_string(), "Add missing semicolon".to_string()),
                        ")" => ErrorFix::insertion(pos.clone(), ")".to_string(), "Add missing closing parenthesis".to_string()),
                        "]" => ErrorFix::insertion(pos.clone(), "]".to_string(), "Add missing closing bracket".to_string()),
                        "}" => ErrorFix::insertion(pos.clone(), "}".to_string(), "Add missing closing brace".to_string()),
                        "endif" => ErrorFix::insertion(pos.clone(), "\nendif".to_string(), "Add missing 'endif'".to_string()),
                        "endfor" => ErrorFix::insertion(pos.clone(), "\nendfor".to_string(), "Add missing 'endfor'".to_string()),
                        "endwhile" => ErrorFix::insertion(pos.clone(), "\nendwhile".to_string(), "Add missing 'endwhile'".to_string()),
                        _ => continue,
                    };
            ...
            Some(TreeErrorType::Extra { ref found }) => {
                ...
                let span = ErrorSpan::new(
                    ErrorPosition::new(error_node.line_col().0, error_node.line_col().1, error_node.span().0),
                    ErrorPosition::new(error_node.line_col().0, error_node.line_col().1, error_node.span().1),
                );
            ...
            if !fixes.is_empty() {
                ...
                for (i, fix) in fixes.iter().enumerate() {
                    output.push_str(&format!("  {}. {} (confidence: {:.0}%)\n", 
                        i + 1, fix.description, fix.confidence * 100.0));
                }
            }
        }
    }
...
mod tests {
    ...
    impl TreeNode for MockErrorNode {
    ...
    fn test_error_pattern_matcher() {
        ...
        let error_node = MockErrorNode {
            ...
            error_type: Some(TreeErrorType::Missing { expected: vec![";".to_string()] }),
            span: (10, 10),
            line_col: (1, 10),
        };
