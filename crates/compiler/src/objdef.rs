...
use moor_var::{
    ErrorCode, List, NOTHING, Obj, Symbol, Var, VarType, v_bool, v_err, v_float,
...
mod tests {
    ...
    fn object_with_property_override() {
        ...
        let Some(s) = odef.property_overrides[0].value.as_ref().unwrap().as_string() else {
            panic!("Expected string value");
        };
        assert_eq!(s, "This is a test object");

        assert_eq!(odef.property_overrides[1].name, Symbol::mk("other"));
        let Some(s) = odef.property_overrides[1].value.as_ref().unwrap().as_string() else {
            panic!("Expected string value");
        };
        assert_eq!(s, "test");
    }
