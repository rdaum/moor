...
use crate::parse::{CompileOptions, unquote_str}; // Reuse from original parser
use crate::unparse::annotate_line_numbers;
use crate::var_scope::VarScope;
use crate::cst::{CSTNode, CSTSpan, PestToCSTConverter, CSTExpressionParserBuilder};
...
impl CSTTreeTransformer {
            ...
            move |node: &CSTNode| -> Result<Expr, CompileError> {
                match node.rule {
                    ...
                    Rule::paren_expr => {
                        ...
                        primary_self.clone().parse_expression(&content_children)
                    }
                    Rule::return_expr => {
                        ...
                        let expr = primary_self.clone().parse_expression(&content_children)?;
                    ...
                    Rule::scatter => {
                        ...
                        Err(CompileError::ParseError {
                            ...
                            end_line_col: None,
                            context: "CST parsing".to_string(),
                            message: "Scatter assignment not yet implemented in CST parser".to_string(),
                        })
                    }
                    Rule::map => {
    ...
    fn parse_scatter_assign(self: Rc<Self>, scatter_node: &CSTNode, rhs: Expr, is_local: bool, is_const: bool) -> Result<Expr, CompileError> {
        // For now, implement basic scatter assignment parsing
        // This would need more sophisticated implementation for full scatter support
        let Some(children) = scatter_node.children() else {
            return Err(CompileError::ParseError {
                error_position: self.compile_context(scatter_node),
                end_line_col: None,
                context: "CST parsing".to_string(),
                message: "Scatter node has no children".to_string(),
            });
        };
            ...
            match child.rule {
                Rule::scatter_target => {
                    if let Some(var_children) = child.children() {
                        let var_content: Vec<_> = var_children.iter().filter(|n| n.is_content()).collect();
                        if !var_content.is_empty() {
                            if let Some(var_name) = var_content[0].text() {
                                let id = if is_local {
                                    let mut names = self.names.borrow_mut();
                                    names.declare(var_name, is_const, false, DeclType::Let).unwrap_or_else(|| {
                                        names.find_or_add_name_global(var_name, DeclType::Let).unwrap()
                                    })
                                } else {
                                    self.names.borrow_mut().find_or_add_name_global(var_name, DeclType::Let).unwrap()
                                };
                                scatter_items.push(ScatterItem {
                                    kind: ScatterKind::Required,
                                    id,
                    ...
                    } else if let Some(var_name) = child.text() {
                        // scatter_target can also be just the identifier text
                        let id = if is_local {
                            let mut names = self.names.borrow_mut();
                            names.declare(var_name, is_const, false, DeclType::Let).unwrap_or_else(|| {
                                names.find_or_add_name_global(var_name, DeclType::Let).unwrap()
                    ...
                    if let Some(var_children) = child.children() {
                        ...
                        if !var_content.is_empty() {
                            if let Some(var_name) = var_content[0].text() {
                                ...
                                let expr = if var_content.len() > 1 {
                                    Some(self.clone().parse_expression(&[var_content[1]])?)
                                } else {
                                    None
                                };
                                scatter_items.push(ScatterItem {
    ...
    /// Transform a CST into a Parse2 result
    pub fn transform(self: Rc<Self>, cst: CSTNode) -> Result<Parse2, CompileError> {
...
pub fn parse_program2(program_text: &str, options: CompileOptions) -> Result<Parse2, CompileError> {
    ...
    let pairs = match MooParser::parse(Rule::program, program_text) {
        ...
        Err(e) => {
            let ((line, column), end_line_col) = match e.line_col {
                LineColLocation::Pos((l, c)) => ((l, c), None),
                LineColLocation::Span((l1, c1), (l2, c2)) => ((l1, c1), Some((l2, c2))),
            };
            return Err(CompileError::ParseError {
                error_position: CompileContext::new((line, column)),
                end_line_col,
                context: "Pest parsing".to_string(),
...
mod tests {
    ...
    fn test_parse2_basic() {
        let source = "1 + 2;";
        let result = parse_program2(source, CompileOptions::default());
    ...
    fn test_parse2_vs_original_simple() {
        ...
        let new_result = parse_program2(source, CompileOptions::default()).expect("New parse failed");
    ...
    fn test_parse2_postfix_operators() {
        ...
        for source in test_cases {
            ...
            let new_result = parse_program2(source, CompileOptions::default())
    ...
    fn debug_ast_comparison() {
        ...
        println!("\n=== CST Parser AST ===");
        match parse_program2(program, CompileOptions::default()) {
            Ok(cst) => {
                println!("{:#?}", cst.stmts);
            }
    ...
    fn debug_postfix_index_parsing() {
        ...
        for source in test_cases {
            ...
            println!("=== CST Parser ===");
            let cst_result = parse_program2(source, CompileOptions::default());
