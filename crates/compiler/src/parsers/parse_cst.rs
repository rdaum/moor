...
use super::parse::{CompileOptions, unquote_str, Parse}; // Reuse from original parser
use crate::unparse::annotate_line_numbers;
use crate::var_scope::VarScope;
use crate::cst::{CSTNode, PestToCSTConverter, CSTExpressionParserBuilder};
...
impl CSTTreeTransformer {
    ...
    #[allow(dead_code)]
    fn create_enhanced_parse_error(&self, source: &str, node: &CSTNode, message: &str, context: ParseContext) -> CompileError {
        ...
        self.error_reporter.create_enhanced_error(source, &enhanced_error)
    }
        ...
        match node.rule {
            Rule::ident => {
                ...
                let name = {
                    ...
                    names_guard.find_or_add_name_global(text.trim(), DeclType::Unknown)
                        .unwrap()
                };
                Ok(Expr::Id(name))
            }
                    ...
                    None => {
                        if self.options.custom_errors {
                            ...
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Custom error codes not yet implemented: {errcode_text}"),
                            });
                        } else {
                            return Err(CompileError::ParseError {
                                error_position: self.compile_context(node),
            ...
            Rule::fn_expr => {
                ...
                let statements = if statements_part.rule == Rule::statements {
                    ...
                    let content_stmts: Vec<_> = stmt_children.iter().filter(|n| n.is_content()).collect();
    ...
    fn parse_expr_node(self: Rc<Self>, expr_node: &CSTNode) -> Result<Expr, CompileError> {
            ...
            if let Some(text) = expr_node.text() {
                if text == ";" {
                    ...
                    return Err(CompileError::ParseError {
                        ...
                        end_line_col: None,
                        context: "CST parsing".to_string(),
                        message: "Parentheses should not be parsed as standalone expressions".to_string(),
                    });
                }
            }
            return Err(CompileError::ParseError {
        ...
        if content_children.len() >= 2 && content_children[0].rule == Rule::scatter_assign {
            ...
            let scatter_content: Vec<_> = scatter_children.iter().filter(|n| n.is_content()).collect();
                ...
                match child.rule {
                    Rule::scatter_target => {
                        ...
                        if let Some(target_children) = child.children() {
                            let target_content: Vec<_> = target_children.iter().filter(|n| n.is_content()).collect();
                            if !target_content.is_empty() {
                                if let Some(var_name) = target_content[0].text() {
                                    if let Some(var) = self.names.borrow_mut().declare(var_name, false, true, DeclType::Assign) {
                    ...
                    Rule::scatter_optional => {
                        ...
                        if let Some(opt_children) = child.children() {
                            let opt_content: Vec<_> = opt_children.iter().filter(|n| n.is_content()).collect();
                    ...
                    Rule::scatter_rest => {
                        ...
                        if let Some(rest_children) = child.children() {
                            let rest_content: Vec<_> = rest_children.iter().filter(|n| n.is_content()).collect();
        ...
        if content_children.len() >= 3 {
            if let (Some(first_text), Some(last_text)) = (content_children[0].text(), content_children[content_children.len() - 1].text()) {
    ...
    fn parse_operand(self: Rc<Self>, node: &CSTNode) -> Result<Expr, CompileError> {
        match node.rule {
            Rule::integer | Rule::float | Rule::string | Rule::boolean 
            | Rule::symbol | Rule::object | Rule::literal_binary 
            | Rule::type_constant | Rule::ident => {
                self.parse_atom(node)
            }
            Rule::list => {
                ...
                if content_children.len() != 1 || content_children[0].rule != Rule::exprlist {
                    return Err(CompileError::ParseError {
                        ...
                        end_line_col: None,
                        context: "CST parsing".to_string(),
                        message: format!("Expected exactly one exprlist in list, found {} children", content_children.len()),
                    });
                }
            ...
            Rule::paren_expr => {
                ...
                if content_children.len() != 1 {
                    return Err(CompileError::ParseError {
                        ...
                        end_line_col: None,
                        context: "CST parsing".to_string(),
                        message: format!("Expected 1 inner expression in parentheses, found {}", content_children.len()),
                    });
                }
            ...
            Rule::for_in_clause => {
                ...
                if content_children.len() != 1 {
                    return Err(CompileError::ParseError {
                        ...
                        end_line_col: None,
                        context: "CST parsing".to_string(),
                        message: format!("Expected 1 expression in for-in clause, found {}", content_children.len()),
                    });
                }
    ...
    fn parse_arglist(self: Rc<Self>, arglist_node: &CSTNode) -> Result<Vec<Arg>, CompileError> {
        ...
        if content_children.len() != 1 {
            return Err(CompileError::ParseError {
                ...
                end_line_col: None,
                context: "CST parsing".to_string(),
                message: format!("Expected 0 or 1 exprlist in arglist, found {}", content_children.len()),
            });
        }
        ...
        if exprlist_node.rule != Rule::exprlist {
            return Err(CompileError::ParseError {
                ...
                end_line_col: None,
                context: "CST parsing".to_string(),
                message: format!("Expected exprlist in arglist, found {:?}", exprlist_node.rule),
            });
        }
    ...
    fn parse_except_codes(self: Rc<Self>, codes_node: &CSTNode) -> Result<CatchCodes, CompileError> {
            ...
            Rule::codes => {
                ...
                if content_children.len() != 1 {
                    return Err(CompileError::ParseError {
                        ...
                        end_line_col: None,
                        context: "CST parsing".to_string(),
                        message: format!("Expected 1 child in codes, found {}", content_children.len()),
                    });
                }
        ...
        if arg_source.starts_with('@') {
            ...
            if content_children.len() == 1 {
                ...
                Err(CompileError::ParseError {
                    ...
                    end_line_col: None,
                    context: "CST parsing".to_string(),
                    message: format!("Normal argument has {} expressions, expected 1", content_children.len()),
                })
            }
        }
    }
    ...
    fn parse_expression(self: Rc<Self>, nodes: &[&CSTNode]) -> Result<Expr, CompileError> {
        ...
        let parser = CSTExpressionParserBuilder::new().build(
            ...
            move |node: &CSTNode| -> Result<Expr, CompileError> {
                match node.rule {
                    // Direct atoms
                    Rule::ident | Rule::type_constant | Rule::object | Rule::integer 
                    | Rule::boolean | Rule::symbol | Rule::float | Rule::string 
                    | Rule::literal_binary | Rule::err | Rule::lambda | Rule::fn_expr => {
                        primary_self.clone().parse_atom(node)
                    }
                    ...
                    Rule::paren_expr => {
                        ...
                        let Some(children) = node.children() else {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: "Parenthesized expression node has no children".to_string(),
                            });
                        };
                        let content_children: Vec<_> = children.iter().filter(|n| n.is_content()).collect();
                        if content_children.is_empty() {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: "Parenthesized expression has no content children".to_string(),
                            });
                        }
                        ...
                        if content_children.len() == 3 {
                            ...
                            if let (Some(left_text), Some(right_text)) = (content_children[0].text(), content_children[2].text()) {
                                if left_text == "(" && right_text == ")" {
                                    // Parse the middle expression
                                    return primary_self.clone().parse_expression(&content_children[1..2]);
                                }
                            }
                        }
                        
                        // Fallback: filter out parentheses nodes from the expression
                        let expr_children: Vec<_> = content_children.into_iter().filter(|n| {
                        ...
                        if expr_children.is_empty() {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: "Parenthesized expression has no non-parentheses children".to_string(),
                            });
                        }
                    ...
                    Rule::return_expr => {
                        ...
                        let expression_children: Vec<_> = content_children.into_iter().filter(|n| {
                        ...
                        let expr = primary_self.clone().parse_expression(&expression_children)?;
                    ...
                    Rule::assign => {
                        ...
                        if content_children.len() == 3 {
                            ...
                            if content_children[0].rule == Rule::scatter {
                                // This is a scatter assignment: {a, b, c} = rhs
                                let right_expr = primary_self.clone().parse_expression(&content_children[2..3])?;
                                primary_self.clone().parse_scatter_assign(content_children[0], right_expr, false, false)
                            } else {
                                let left_expr = primary_self.clone().parse_expression(&content_children[0..1])?;
                                let right_expr = primary_self.clone().parse_expression(&content_children[2..3])?;
                                Ok(Expr::Assign {
                            ...
                            if let Some(eq_pos) = equals_pos {
                                if eq_pos + 1 < content_children.len() {
                                    let right_expr = primary_self.clone().parse_expression(&content_children[eq_pos + 1..])?;
                            ...
                            Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected at least 3 assignment children, found {}", content_children.len()),
                            })
                        }
                    }
                    Rule::cond_expr => {
                        ...
                        if content_children.len() != 3 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected 3 conditional children, found {}", content_children.len()),
                            });
                        }
                        let condition = primary_self.clone().parse_expression(&content_children[0..1])?;
                        let true_expr = primary_self.clone().parse_expression(&content_children[1..2])?;
                        let false_expr = primary_self.clone().parse_expression(&content_children[2..3])?;
                        Ok(Expr::Cond {
                    ...
                    Rule::pass_expr => {
                        ...
                        let args = if !content_children.is_empty() && content_children[0].rule == Rule::exprlist {
                    ...
                    Rule::try_expr => {
                        ...
                        let try_expr = primary_self.clone().parse_expression(&[content_children[0]])?;
                        ...
                        let except = if content_children.len() > 2 {
                            Some(Box::new(primary_self.clone().parse_expression(&[content_children[2]])?))
                        } else {
                            None
                        };
                    ...
                    Rule::builtin_call => {
                        ...
                        if content_children.len() < 2 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: "Builtin call missing function name or arguments".to_string(),
                            });
                        }
                        ...
                        let call_target = if BUILTINS.find_builtin(symbol).is_some() {
                            ...
                            let var_name = primary_self.names
                                .borrow_mut()
                    ...
                    Rule::sysprop_call => {
                        ...
                        if content_children.len() < 2 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: "System property call missing property or arguments".to_string(),
                            });
                        }
                    ...
                    Rule::scatter => {
                        ...
                        Err(CompileError::ParseError {
                            ...
                            end_line_col: None,
                            context: "CST parsing".to_string(),
                            message: "Scatter patterns can only appear on the left side of assignments".to_string(),
                        })
                    }
                    Rule::map => {
                        ...
                        let pairs = elements
                            ...
                            .map(|pair| {
                                ...
                                let value = pair.get(1).cloned().unwrap_or_else(|| Expr::Value(v_none()));
                                (key, value)
                            })
                            .collect();
                        Ok(Expr::Map(pairs))
                    ...
                    Rule::for_iterable => {
                        ...
                        if content_children.len() != 1 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected 1 expression in for-iterable, found {}", content_children.len()),
                            });
                        }
                        ...
                        match child.rule {
                            Rule::for_range_clause => {
                                ...
                                let range_content: Vec<_> = range_children.iter().filter(|n| n.is_content()).collect();
                                ...
                                let start = primary_self.clone().parse_expression(&[range_content[0]])?;
                                let end = primary_self.clone().parse_expression(&[range_content[1]])?;
                    ...
                    Rule::exprlist => {
                        ...
                        Err(CompileError::ParseError {
                            ...
                            end_line_col: None,
                            context: "CST parsing".to_string(),
                            message: "exprlist should not be handled as primary expression".to_string(),
                        })
                    }
                    Rule::flyweight => {
                            ...
                            match child.rule {
                                Rule::flyweight_slots => {
                                    ...
                                    if let Some(slot_children) = child.children() {
                                        let slot_content: Vec<_> = slot_children.iter().filter(|n| n.is_content()).collect();
                                        let mut i = 0;
                                            ...
                                            if let Some(slot_name_text) = slot_name_node.text() {
                                                ...
                                                if slot_name == Symbol::mk("delegate") || slot_name == Symbol::mk("slots") {
                                                    return Err(CompileError::BadSlotName(
                                                        primary_self.compile_context(slot_name_node),
                                                        slot_name.to_string(),
                                                    ));
                                                }
                                                let slot_expr = primary_self.clone().parse_operand(slot_expr_node)?;
                                ...
                                _ => {
                                    // Contents expression
                                    contents = Some(Box::new(primary_self.clone().parse_operand(child)?));
                                }
                            }
                        }
                    ...
                    Rule::range_comprehension => {
                        ...
                        match range_clause.rule {
                            Rule::for_range_clause => {
                                ...
                                if range_content.len() != 2 {
                                    return Err(CompileError::ParseError {
                                        ...
                                        end_line_col: None,
                                        context: "CST parsing".to_string(),
                                        message: format!("Expected 2 range expressions, found {}", range_content.len()),
                                    });
                                }
                                ...
                                let variable = {
                                    ...
                                    let Some(name) = names.declare_name(varname.trim(), DeclType::For) else {
                                ...
                                let producer_expr = primary_self.clone().parse_operand(content_children[0])?;
                                // Enter scope for registers only
                                primary_self.enter_scope();
                                let start_expr = primary_self.clone().parse_operand(range_content[0])?;
                                let end_expr = primary_self.clone().parse_operand(range_content[1])?;
                                let end_of_range_register = primary_self.names.borrow_mut().declare_register()?;
                            ...
                            Rule::for_in_clause => {
                                ...
                                let in_content: Vec<_> = in_children.iter().filter(|n| n.is_content()).collect();
                                if in_content.len() != 1 {
                                    return Err(CompileError::ParseError {
                                        error_position: primary_self.compile_context(range_clause),
                                        end_line_col: None,
                                        context: "CST parsing".to_string(),
                                        message: format!("Expected 1 expression in for-in clause, found {}", in_content.len()),
                                    });
                                }
                                ...
                                // Enter scope for registers only
                                primary_self.enter_scope();
                                let list_expr = primary_self.clone().parse_operand(in_content[0])?;
                                let position_register = primary_self.names.borrow_mut().declare_register()?;
                                let list_register = primary_self.names.borrow_mut().declare_register()?;
                                primary_self.exit_scope();
                            ...
                            _ => {
                                Err(CompileError::ParseError {
                                    ...
                                    end_line_col: None,
                                    context: "CST parsing".to_string(),
                                    message: format!("Unimplemented comprehension clause: {:?}", range_clause.rule),
                                })
                            }
                        }
                    }
            ...
            move |left: Expr, op: &CSTNode, right: Expr| -> Result<Expr, CompileError> {
                match op.rule {
                    ...
                    Rule::modulus => Ok(Expr::Binary(BinaryOp::Mod, Box::new(left), Box::new(right))),
                    ...
                    Rule::in_range => Ok(Expr::Binary(BinaryOp::In, Box::new(left), Box::new(right))),
                        ...
                        for child in content_children {
                            match child.rule {
                                Rule::scatter_target => {
                                    ...
                                    let Some(var) = prefix_self.names.borrow_mut().declare(var_name, false, true, DeclType::Assign) else {
                                        return Err(CompileError::ParseError {
                                            error_position: prefix_self.compile_context(op),
                                            end_line_col: None,
                                            context: "CST parsing".to_string(),
                                            message: format!("Failed to declare scatter variable: {var_name}"),
                                        });
                                    };
                                ...
                                Rule::scatter_optional => {
                                    ...
                                    if !opt_content.is_empty() {
                                        ...
                                        // Check for default value  
                                        let default_expr = if opt_content.len() > 1 {
                                            Some(prefix_self.clone().parse_expression(&opt_content[1..])?)
                                        } else {
                                            None
                                        };
                    ...
                    Rule::index_single => {
                        ...
                        if content_children.len() != 1 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected 1 index expression, found {}", content_children.len()),
                            });
                        }
                    ...
                    Rule::index_range => {
                        ...
                        if content_children.len() != 2 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected 2 range expressions, found {}", content_children.len()),
                            });
                        }
                    ...
                    Rule::prop => {
                        ...
                        if content_children.len() != 1 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected 1 property identifier, found {}", content_children.len()),
                            });
                        }
                    ...
                    Rule::prop_expr => {
                        ...
                        if content_children.len() != 1 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected 1 property expression, found {}", content_children.len()),
                            });
                        }
                    ...
                    Rule::verb_call => {
                        ...
                        if content_children.len() != 2 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected verb name and arglist, found {}", content_children.len()),
                            });
                        }
                    ...
                    Rule::verb_expr_call => {
                        ...
                        if content_children.len() != 2 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected verb expression and arglist, found {}", content_children.len()),
                            });
                        }
                    ...
                    Rule::cond_expr => {
                        ...
                        if content_children.len() != 2 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected true and false expressions, found {}", content_children.len()),
                            });
                        }
                    ...
                    Rule::assign => {
                        ...
                        if content_children.len() != 1 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: format!("Expected 1 assignment expression, found {}", content_children.len()),
                            });
                        }
        ...
        match node.rule {
            ...
            Rule::expr_statement => {
                ...
                match first_child.rule {
                    Rule::for_statement | Rule::for_in_statement | Rule::for_range_statement 
                    | Rule::while_statement | Rule::labelled_while_statement
                    | Rule::if_statement | Rule::try_except_statement | Rule::try_finally_statement
                    | Rule::break_statement | Rule::continue_statement | Rule::fork_statement
                    | Rule::labelled_fork_statement | Rule::local_assignment | Rule::const_assignment
                    | Rule::global_assignment | Rule::fn_statement | Rule::begin_statement => {
            ...
            Rule::empty_return => {
                Ok(Some(Stmt::new(StmtNode::Expr(Expr::Return(None)), line_col)))
            }
            Rule::expr => {
            ...
            Rule::if_statement => {
                ...
                while idx < content_children.len() {
                    ...
                    match clause.rule {
                        Rule::elseif_clause => {
                            ...
                            let elseif_content: Vec<_> = elseif_children.iter().filter(|n| n.is_content()).collect();
                            ...
                            self.enter_scope();
                            let body = self.clone().parse_statements_from_node(elseif_content[1])?;
                            let environment_width = self.exit_scope();
                        ...
                        Rule::else_clause => {
                            ...
                            let else_content: Vec<_> = else_children.iter().filter(|n| n.is_content()).collect();
                            ...
                            self.enter_scope();
                            let statements = self.clone().parse_statements_from_node(else_content[0])?;
                            let environment_width = self.exit_scope();
                ...
                Ok(Some(Stmt::new(StmtNode::Cond { arms, otherwise }, line_col)))
            }
            Rule::while_statement => {
                ...
                self.enter_scope();
                let body = self.clone().parse_statements_from_node(content_children[1])?;
                let environment_width = self.exit_scope();

                Ok(Some(Stmt::new(StmtNode::While {
                    id: None,
                    condition,
                    body,
                    environment_width,
            ...
            Rule::for_statement => {
                ...
                if let Some(fields) = node.fields() {
                    // Get variable field
                    let variable_node = fields.get("variable").ok_or_else(|| CompileError::ParseError {
                    ...
                    let variable_text = variable_node.text().ok_or_else(|| CompileError::ParseError {
                        error_position: context.clone(),
                        end_line_col: None,
                        context: "CST parsing".to_string(),
                    ...
                    // Get iterable field
                    let iterable_node = fields.get("iterable").ok_or_else(|| CompileError::ParseError {
                        error_position: context.clone(),
                        end_line_col: None,
                        context: "CST parsing".to_string(),
                    ...
                    if let Some(iterable_fields) = iterable_node.fields() {
                        if iterable_fields.contains_key("start") && iterable_fields.contains_key("end") {
                            // This is a range: for i in [start..end]
                            let start_expr = self.clone().parse_expression(&[iterable_fields.get("start").unwrap()])?;
                            let end_expr = self.clone().parse_expression(&[iterable_fields.get("end").unwrap()])?;
                            
                            // Declare loop variable
                            let id = self.names.borrow_mut().declare_or_use_name(variable_text, DeclType::For);
                            
                            // Parse body
                            let body_node = fields.get("body").ok_or_else(|| CompileError::ParseError {
                                error_position: context.clone(),
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                            ...
                            return Ok(Some(Stmt::new(StmtNode::ForRange {
                                id,
                                from: start_expr,
                                to: end_expr,
                                body,
                            ...
                            let iter_expr = self.clone().parse_expression(&[iterable_fields.get("expression").unwrap()])?;
                            
                            // Declare loop variable
                            let value_binding = self.names.borrow_mut().declare_or_use_name(variable_text, DeclType::For);
                            ...
                            return Ok(Some(Stmt::new(StmtNode::ForList {
                                value_binding,
                                key_binding: None,
                                expr: iter_expr,
                                body,
                ...
                match iterable_node.rule {
                    Rule::for_range_clause => {
                        ...
                        Ok(Some(Stmt::new(StmtNode::ForRange {
                            id,
                            from: start_expr,
                            to: end_expr,
                            body,
                    ...
                    Rule::for_in_clause => {
                        ...
                        Ok(Some(Stmt::new(StmtNode::ForList {
                            value_binding,
                            key_binding: None,
                            expr: iter_expr,
                            body,
                    ...
                    Rule::for_iterable => {
                        ...
                        let iterable_content: Vec<_> = iterable_children.iter().filter(|n| n.is_content()).collect();
            ...
            Rule::for_in_statement => {
                ...
                let index_content: Vec<_> = index_children.iter().filter(|n| n.is_content()).collect();
                ...
                let value_binding = self.names.borrow_mut().declare_or_use_name(value_var_text, DeclType::For);
                let key_binding = key_var.map(|key_text| self.names.borrow_mut().declare_or_use_name(key_text, DeclType::For));
            ...
            Rule::for_range_statement => {
                ...
                let var_name = if let Some(var_text) = var_node.text() {
                    ...
                    let var_content: Vec<_> = var_children.iter().filter(|n| n.is_content()).collect();
                ...
                let var_name_obj = self.names.borrow_mut().declare_or_use_name(var_name, DeclType::For);
            ...
            Rule::try_except_statement => {
                ...
                for except_child in content_children.iter().skip(1) {
                    if except_child.rule == Rule::except {
                        ...
                        let except_content: Vec<_> = except_children.iter().filter(|n| n.is_content()).collect();
                        ...
                        let (id, codes) = match except_clause.rule {
                            Rule::labelled_except => {
                                ...
                                let clause_content: Vec<_> = clause_children.iter().filter(|n| n.is_content()).collect();
                                if clause_content.len() < 2 {
                                    continue;
                                }
                                let var_name = clause_content[0].text().unwrap_or("e");
                                let exception_id = Some(self.names.borrow_mut().declare_or_use_name(var_name, DeclType::Except));
                        ...
                        let statements = self.clone().parse_statements_from_node(except_statements)?;
                ...
                Ok(Some(Stmt::new(StmtNode::TryExcept {
                    body,
                    excepts,
                    environment_width,
                }, line_col)))
            ...
            Rule::try_finally_statement => {
                ...
                Ok(Some(Stmt::new(StmtNode::TryFinally {
                    body,
                    handler,
                    environment_width,
                }, line_col)))
            ...
            Rule::break_statement => {
                ...
                let label_var = self.names.borrow_mut().find_or_add_name_global(label_name, DeclType::Unknown).unwrap();
                Ok(Some(Stmt::new(StmtNode::Break { exit: Some(label_var) }, line_col)))
            }
            Rule::continue_statement => {
                ...
                let label_var = self.names.borrow_mut().find_or_add_name_global(label_name, DeclType::Unknown).unwrap();
                Ok(Some(Stmt::new(StmtNode::Continue { exit: Some(label_var) }, line_col)))
            }
            Rule::fork_statement => {
                let Some(children) = node.children() else {
                    return Err(CompileError::ParseError {
                ...
                if content_children[idx].rule == Rule::ident {
                    ...
                    id = Some(self.names.borrow_mut().find_or_add_name_global(label_name, DeclType::Unknown).unwrap());
                    idx += 1;
                }
                ...
                Ok(Some(Stmt::new(StmtNode::Fork {
                    id,
                    time: time_expr,
                    body,
                }, line_col)))
            ...
            Rule::labelled_fork_statement => {
                ...
                let id = Some(self.names.borrow_mut().find_or_add_name_global(label_name.trim(), DeclType::Unknown).unwrap());
            ...
            Rule::local_assignment | Rule::const_assignment => {
                ...
                match parts.rule {
                    ...
                    Rule::local_assign_scatter | Rule::const_assign_scatter => {
                        ...
                        let assign_content: Vec<_> = assign_children.iter().filter(|n| n.is_content()).collect();
                        ...
                        let expr = self.parse_scatter_assign(scatter_op, rhs_expr, true, is_const)?;
                        Ok(Some(Stmt::new(StmtNode::Expr(expr), line_col)))
                    }
                    _ => {
                        Err(CompileError::ParseError {
            ...
            Rule::labelled_while_statement => {
                ...
                let Some(id) = self.names.borrow_mut().declare_name(varname, DeclType::WhileLabel) else {
                ...
                self.enter_scope();
                let body = self.clone().parse_statements_from_node(content_children[2])?;
                let environment_width = self.exit_scope();
            ...
            Rule::begin_statement => {
                ...
                self.enter_scope();
                let body = self.clone().parse_statements_from_node(content_children[0])?;
                let num_bindings = self.exit_scope();
                
                Ok(Some(Stmt::new(StmtNode::Scope {
                    num_bindings,
                    body,
                }, line_col)))
            }
            ...
            Rule::fn_statement => {
                ...
                match inner.rule {
                    Rule::fn_named => {
                        ...
                        let fn_content: Vec<_> = fn_children.iter().filter(|n| n.is_content()).collect();
                        if fn_content.len() < 3 {
                            return Err(CompileError::ParseError {
                                error_position: self.compile_context(inner),
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: "Fn named statement missing name, params, or body".to_string(),
                            });
                        }

                        let func_name = fn_content[0].text().ok_or_else(|| CompileError::ParseError {
                        ...
                        self.enter_scope();
                        let statements = self.clone().parse_statements_from_node(statements_part)?;
                    ...
                    Rule::fn_assignment => {
                        ...
                        if fn_content.len() < 2 {
                            return Err(CompileError::ParseError {
                                ...
                                end_line_col: None,
                                context: "CST parsing".to_string(),
                                message: "Fn assignment statement missing variable or function".to_string(),
                            });
                        }

                        let var_name = fn_content[0].text().ok_or_else(|| CompileError::ParseError {
                            error_position: self.compile_context(fn_content[0]),
                            end_line_col: None,
                        ...
                        let func_content: Vec<_> = func_children.iter().filter(|n| n.is_content()).collect();
                        ...
                        let assign_expr = match maybe_id {
                            ...
                            None => {
                                ...
                                let id = {
                                    let mut names = self.names.borrow_mut();
                                    let Some(id) = names.declare_name(var_name, DeclType::Let) else {
                                        return Err(CompileError::ParseError {
                                            error_position: context,
                                            end_line_col: None,
                                            context: "fn assignment parsing".to_string(),
                                            message: format!("Could not declare variable: {var_name}"),
                                        });
                                    };
                                    id
                                };
    ...
    fn parse_statements_from_node(self: Rc<Self>, node: &CSTNode) -> Result<Vec<Stmt>, CompileError> {
        match node.rule {
            Rule::statements => {
                let Some(children) = node.children() else {
                    return Ok(vec![]);
    ...
    fn parse_scatter_assign(self: Rc<Self>, scatter_node: &CSTNode, rhs: Expr, is_local: bool, is_const: bool) -> Result<Expr, CompileError> {
        ...
        for child in content_children {
            match child.rule {
                Rule::ident => {
                    ...
                    let id = if is_local {
                        let mut names = self.names.borrow_mut();
                        names.declare(var_name, is_const, false, DeclType::Assign).unwrap_or_else(|| {
                            names.find_or_add_name_global(var_name, DeclType::Assign).unwrap()
                        })
                    } else {
                        self.names.borrow_mut().find_or_add_name_global(var_name, DeclType::Assign).unwrap()
                    };
                ...
                Rule::scatter_optional => {
                    ...
                    let default_expr = if opt_content.len() >= 2 && opt_content[1].rule != Rule::ident {
    ...
    fn parse_lambda_params(self: Rc<Self>, params_node: &CSTNode) -> Result<Vec<ScatterItem>, CompileError> {
        ...
        for child in content_children {
            match child.rule {
                Rule::lambda_param => {
                    ...
                    let param_content: Vec<_> = param_children.iter().filter(|n| n.is_content()).collect();
        ...
        let statements = match cst.rule {
            Rule::program => {
                ...
                for child in content_children {
                    match child.rule {
                        Rule::statements => {
                            let parsed_statements = self.clone().parse_statements_from_node(child)?;
...
pub fn parse_program_cst(program_text: &str, options: CompileOptions) -> Result<ParseCst, CompileError> {
    ...
    let cst = converter.convert_program(program_pair)
...
mod tests {
    ...
    fn test_parse2_basic() {
        ...
        match &parse_result.stmts[0].node {
            ...
            _ => panic!("Expected expression statement, got: {:?}", parse_result.stmts[0].node),
        }
    }
    ...
    fn test_parse2_vs_original_simple() {
        ...
        let original_result = parse_program(source, CompileOptions::default()).expect("Original parse failed");
        
        // Parse with new parser
        let new_result = parse_program_cst(source, CompileOptions::default()).expect("New parse failed");
        
        // Compare statement counts
        assert_eq!(new_result.stmts.len(), original_result.stmts.len(), "Statement count mismatch");
            ...
            match (&original_stmt.node, &new_stmt.node) {
                (StmtNode::Expr(orig_expr), StmtNode::Expr(new_expr)) => {
                    ...
                    match (orig_expr, new_expr) {
                        ...
                        _ => {
                            panic!("Expression mismatch: original={:?}, new={:?}", orig_expr, new_expr);
                        }
                    }
                }
                _ => {
                    panic!("Statement type mismatch: original={:?}, new={:?}", original_stmt.node, new_stmt.node);
                }
            }
        }
    }
    ...
    fn test_parse2_postfix_operators() {
        ...
        for source in test_cases {
            ...
            assert!(!new_result.stmts.is_empty(), "No statements parsed for: {}", source);
        }
    }
    ...
    fn test_parse2_comprehensive_ast_parity() {
        ...
        for source in test_cases {
            ...
            assert_eq!(new_result.cst.to_source(), source, "Source preservation failed for: {}", source);
        }
    }
    ...
    fn test_parse2_vs_original_multiple_expressions() {
        ...
        for source in test_cases {
            ...
            assert_eq!(new_result.cst.to_source(), source, "CST source preservation failed for: {}", source);
            ...
            if original_result.stmts.len() == 1 && new_result.stmts.len() == 1 {
                ...
                match (&original_stmt.node, &new_stmt.node) {
                    ...
                    _ => {
                        panic!("Statement type mismatch for {}: original={:?}, new={:?}", 
    ...
    fn test_cst_expression_parsing() {
        ...
        let left = CSTNode::terminal(
            Rule::integer,
            "1".to_string(),
            CSTSpan { start: 0, end: 1, line_col: (1, 1) },
        );
        let op = CSTNode::terminal(
            Rule::add,
            "+".to_string(),
            CSTSpan { start: 2, end: 3, line_col: (1, 3) },
        );
        let right = CSTNode::terminal(
            Rule::integer,
            "2".to_string(),
            CSTSpan { start: 4, end: 5, line_col: (1, 5) },
        );
    ...
    fn debug_postfix_index_parsing() {
        ...
        for source in test_cases {
            ...
            match (original_result, cst_result) {
                (Ok(orig), Ok(cst)) => {
                    if let (Some(orig_stmt), Some(cst_stmt)) = (orig.stmts.first(), cst.stmts.first()) {
                        if let (StmtNode::Expr(orig_expr), StmtNode::Expr(cst_expr)) = (&orig_stmt.node, &cst_stmt.node) {
                            println!("Original: {:?}", orig_expr);
                            println!("CST:      {:?}", cst_expr);
                            
                            // Check if they're structurally the same
                            let same_type = std::mem::discriminant(orig_expr) == std::mem::discriminant(cst_expr);
    ...
    fn test_debug_conditional_cst() {
        ...
        let converter = PestToCSTConverter::new(source.to_string());
        let cst = converter.convert_program(pairs.into_iter().next().unwrap()).expect("Failed to convert to CST");
