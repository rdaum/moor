...
pub fn get_available_parsers() -> Vec<Box<dyn MooParser>> {
    let mut parsers: Vec<Box<dyn MooParser>> = vec![
        Box::new(OriginalPestParser),
        Box::new(CstParser),
    ];
...
mod tests {
    ...
    fn test_simple_compilation() {
        ...
        for parser in get_available_parsers() {
            ...
            assert!(result.is_ok(), "Parser '{}' failed to compile simple program: {:?}", 
                parser.name(), result.err());
        }
    }
    ...
    fn test_for_loop_compilation() {
        ...
        for parser in get_available_parsers() {
            let result = parser.compile(program, options.clone());
            assert!(result.is_ok(), "Parser '{}' failed to compile for loop: {:?}", 
                parser.name(), result.err());
        }
    }
    ...
    fn test_parser_consistency() {
        ...
        for program in test_programs {
            ...
            assert!(success_count == 0 || success_count == parsers.len(),
                "Inconsistent parser results for '{}': {:?}", program, 
                results.iter().map(|(name, r)| (name, r.is_ok())).collect::<Vec<_>>());
        }
    }
}