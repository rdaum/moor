...
use crate::errors::tree_error_recovery::{TreeErrorInfo, TreeErrorType, ErrorFix, ErrorPosition, ErrorSpan};
...
impl<'a> TreeSitterConverter<'a> {
    ...
    pub fn parse(&mut self) -> Result<CSTNode, CompileError> {
        ...
        parser.set_language(&tree_sitter_moo::language())
    ...
    fn categorize_node(&self, node_kind: &str) -> NodeCategory {
        match node_kind {
            ...
            "statement" | "expression_statement" | "assignment_operation" | "assignment_expr" |
            "try_statement" | "if_statement" | "while_statement" | "for_statement" | 
            "for_in_statement" | "return_statement" | "return_expression" | 
            "break_statement" | "continue_statement" => NodeCategory::Statement,
            
            "expression" | "binary_operation" | "unary_operation" | "conditional_operation" |
            "method_call" | "function_call" | "call" | "property" | "index" | 
    ...
    fn convert_punctuation(&self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        Ok(CSTNode {
            rule: Rule::expr,
            span,
            kind: CSTNodeKind::Terminal { text: self.node_text(node) },
        })
    }
    
    /// Convert program-level nodes
    fn convert_program_node(&mut self, node_kind: &str, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        match node_kind {
    ...
    fn convert_statement_node(&mut self, node_kind: &str, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        match node_kind {
    ...
    fn convert_expression_node(&mut self, node_kind: &str, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        match node_kind {
    ...
    fn convert_terminal_node(&mut self, node_kind: &str, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
            ...
            "error_code" => {
                ...
                if let Some(child) = node.child(0) {
                    return Ok(CSTNode {
                        rule: Rule::errcode,
                        span,
                        kind: CSTNodeKind::Terminal { text: self.node_text(&child) },
                    });
                } else {
                    return Err(CompileError::ParseError {
                        error_position: CompileContext { 
                            line_col: (node.start_position().row + 1, node.start_position().column + 1) 
                        },
                        end_line_col: None,
    ...
    fn convert_collection_node(&mut self, node_kind: &str, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        match node_kind {
    ...
    fn convert_system_node(&mut self, node_kind: &str, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        match node_kind {
    ...
    fn convert_error_node(&mut self, node: &Node, _span: CSTSpan) -> Result<CSTNode, CompileError> {
            ...
            // Look for patterns in the whole context
                ...
                if let Some(colon_pos) = context_text.rfind(':') {
                    if colon_pos < error_offset && error_offset - colon_pos < 3 {
                        "Incomplete method call - add method name and parentheses after ':'".to_string()
                    } else {
                        TreeErrorInfo::error_message(node)
                    }
                } else {
        ...
        if !fixes.is_empty() {
            message.push_str("\nSuggested fixes:");
            for fix in fixes.iter().take(3) { // Limit to 3 suggestions
                message.push_str(&format!("\n  • {}", fix.description));
            }
        }
    ...
    #[allow(dead_code)]
    fn wrap_as_statement(&mut self, parts: Vec<CSTNode>, _end_pos: usize) -> Result<CSTNode, CompileError> {
        if parts.is_empty() {
            return Err(CompileError::ParseError {
            ...
            match part.rule {
                Rule::assign => {
                    ...
                    CSTNode {
                        ...
                        kind: CSTNodeKind::NonTerminal { children: vec![part] },
                    }
                }
                _ => part,
            }
        ...
        Ok(CSTNode {
            rule: Rule::statement,
            span: stmt_span,
            kind: CSTNodeKind::NonTerminal { children: vec![content] },
        })
    }
    ...
    fn convert_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        ...
        let wrapped_content = match content_node.kind() {
            ...
            "if_statement" | "while_statement" | "for_statement" | "for_in_statement" | "try_statement" => {
                content
            },
            // These need to be wrapped as expression statements
            "expression" | "assignment_operation" | "assignment_expr" | "method_call" | 
            "function_call" | "call" | "binary_operation" | "unary_operation" => {
                CSTNode {
                    rule: Rule::expr_statement,
        ...
        Ok(CSTNode {
            rule: Rule::statement,
            span,
            kind: CSTNodeKind::NonTerminal { children: vec![wrapped_content] },
        })
    }
    
    /// Convert expression statement using field access
    fn convert_expression_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        ...
        let expr_node = if let Some(expr) = node.child_by_field_name("expression") {
            ...
            node.children(&mut node.walk())
                .find(|child| matches!(child.kind(), 
                    "expression" | "assignment_operation" | "assignment_expr" | 
                    "method_call" | "function_call" | "call"))
                .ok_or_else(|| self.error_at_position(
                    node.start_position().row,
                    node.start_position().column,
        ...
        Ok(CSTNode {
            rule: Rule::expr_statement,
            span,
            kind: CSTNodeKind::NonTerminal { children: vec![expr] },
        })
    }
    ...
    fn extract_method_call_parts(&mut self, node: &Node) -> Result<(Option<CSTNode>, Option<CSTNode>, Vec<CSTNode>), CompileError> {
    ...
    fn build_sysprop_call(&mut self, object: CSTNode, method: Option<CSTNode>, args: Vec<CSTNode>, span: CSTSpan) -> Result<CSTNode, CompileError> {
    ...
    fn build_verb_call(&mut self, object: Option<CSTNode>, method: Option<CSTNode>, args: Vec<CSTNode>, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_system_property(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
    ...
    fn convert_terminal(&mut self, node: &Node, span: CSTSpan, rule: Rule) -> Result<CSTNode, CompileError> {
        Ok(CSTNode {
            rule,
            span,
            kind: CSTNodeKind::Terminal { text: self.node_text(node) },
    ...
    fn convert_expression(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        ...
        if let Some(child) = node.child(0) {
            ...
            match child.kind() {
                ...
                "assignment_operation" | "assignment_expr" => self.convert_assignment(&child, child_span),
                ...
                "parenthesized_expression" => self.convert_parenthesized_expression(&child, child_span),
                ...
                _ => {
                    ...
                    match child_node.rule {
                        Rule::assign | Rule::verb_call | Rule::builtin_call | Rule::sysprop_call => {
                        ...
                        _ => {
                            ...
                            Ok(CSTNode {
                                rule: Rule::expr,
                                span,
                                kind: CSTNodeKind::NonTerminal { children: vec![child_node] },
                            })
                        }
                    }
                }
    ...
    fn convert_return_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_break_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        ...
        Ok(CSTNode {
            ...
            span,
            kind: CSTNodeKind::Terminal { text: "break".to_string() },
        })
    }
    
    fn convert_continue_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        if node.child_count() > 0 {
            return Err(self.error_at_position(
                node.start_position().row,
                node.start_position().column,
        ...
        Ok(CSTNode {
            ...
            span,
            kind: CSTNodeKind::Terminal { text: "continue".to_string() },
        })
    }
    
    fn convert_if_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
    ...
    fn convert_while_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_for_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_try_statement(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_except_clause(&mut self, node: &Node) -> Result<CSTNode, CompileError> {
        ...
        if let Some(codes) = node.child_by_field_name("codes") {
            ...
            let exprlist_node = CSTNode {
                ...
                kind: CSTNodeKind::NonTerminal { children: exprlist_children },
            };
            ...
            let codes_node = CSTNode {
                rule: Rule::codes,
                span: codes_span.clone(),
                kind: CSTNodeKind::NonTerminal { children: codes_children },
            };
            ...
            let unlabelled_except = CSTNode {
                ...
                span: codes_span,
                kind: CSTNodeKind::NonTerminal { children: except_clause_children },
            };
        ...
        if let Some(body) = node.child_by_field_name("body") {
            ...
            let statements_node = CSTNode {
                rule: Rule::statements,
                span: body_span,
                kind: CSTNodeKind::NonTerminal { children: vec![body_cst] },
            };
    ...
    fn convert_binary_operation(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_unary_operation(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_conditional_operation(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_function_call(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_property_access(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn convert_parenthesized_expression(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        ...
        if let Some(inner) = node.child(0) {
            if inner.kind() == "expression" {
                ...
                return Ok(CSTNode {
                    ...
                    span,
                    kind: CSTNodeKind::NonTerminal { children: vec![inner_expr] },
                });
            }
        }
    ...
    fn convert_list(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        ...
        let exprlist = CSTNode {
            rule: Rule::exprlist,
            span: span.clone(),
            kind: CSTNodeKind::NonTerminal { children: arguments },
        };
                ...
                if let Some(key) = child.child_by_field_name("key") {
                    ...
                    let key_expr = if key_node.rule == Rule::expr {
                        ...
                        CSTNode {
                            ...
                            kind: CSTNodeKind::NonTerminal { children: vec![key_node] },
                        }
                    };
                ...
                if let Some(value) = child.child_by_field_name("value") {
                    ...
                    let value_expr = if value_node.rule == Rule::expr {
                        ...
                        CSTNode {
                            ...
                            kind: CSTNodeKind::NonTerminal { children: vec![value_node] },
                        }
                    };
    ...
    fn convert_unknown_node(&mut self, node: &Node, span: CSTSpan) -> Result<CSTNode, CompileError> {
        let mut children = Vec::new();
    ...
    fn find_error(&self, node: &Node) -> CompileError {
        ...
        if node.kind() == "ERROR" {
            ...
            let message = self.analyze_error_with_semantic_walker(node)
    ...
    fn analyze_error_with_semantic_walker(&self, node: &Node) -> Option<String> {
        ...
        if walker.discover_semantics(&root).is_ok() &&
           walker.analyze_semantics().is_ok() {
    ...
    fn analyze_error_with_semantic_context(&self, _walker: &super::tree_walker::SemanticTreeWalker, node: &Node) -> Option<String> {
        // Check if this is a property access or method call error
        if let Some(parent) = node.parent() {
            match parent.kind() {
                "property_access" | "member_access" => {
                    Some("Incomplete property access - property name required after '.'".to_string())
                }
                ...
                "expression_statement" => {
                    ...
                    if context.contains(".") {
                        ...
                        Some("Incomplete method call - add method name and arguments after ':'".to_string())
                    } else {
                        None
                    }
                }
    ...
    fn analyze_error_with_source_pattern(&self, node: &Node) -> String {
        ...
        } else if context.contains("(") && context.matches("(").count() > context.matches(")").count() {
...
pub fn parse_program_with_tree_sitter(source: &str, options: crate::CompileOptions) -> Result<crate::parsers::parse_cst::ParseCst, CompileError> {
    ...
    fn parse_context(&self) -> ParseContext {
        ...
        match self.kind() {
            ...
            _ => {
                ...
                if let Some(parent) = self.parent() {
                    match parent.kind() {
                        "property" | "property_access" => ParseContext::Unknown("PropertyAccess".to_string()),
                        "method_call" | "verb_call" => ParseContext::Unknown("MethodCall".to_string()),
                        "statement" => ParseContext::Statement,
                        "expression" => ParseContext::Expression,
                        "assignment_operation" => ParseContext::Assignment,
                        "if_statement" => ParseContext::IfStatement,
...
fn get_expected_tokens_for_parent(parent_kind: &str) -> Vec<String> {
    match parent_kind {
        ...
        "try_statement" => vec!["except".to_string(), "finally".to_string(), "endtry".to_string()],
        "expression" => vec![
            "identifier".to_string(),
            "number".to_string(),
            "string".to_string(),
        ...
        "property" | "property_access" => vec![
            "property name".to_string(),
        ],
        "method_call" | "verb_call" => vec![
