...
pub type NodeHandler<T> = std::sync::Arc<
    dyn Fn(&T, &mut dyn ASTBuilderContext<T>) -> Result<Expr, CompileError> + Send + Sync,
>;
...
impl<T: TreeNode> NodeHandlerRegistry<T> {
    ...
    {
        self.handlers
            .insert(node_kind.to_string(), std::sync::Arc::new(handler));
    }
    ...
    pub fn register_default_handlers(&mut self) {
        ...
        self.register("string_literal", |node, builder| {
            ...
            let parsed = crate::parsers::parse::unquote_str(text).map_err(|e| {
                builder.parse_error(node, &format!("Invalid string literal: {}", e))
            })?;
