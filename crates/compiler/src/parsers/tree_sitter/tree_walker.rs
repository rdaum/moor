...
impl<'a> SemanticTreeWalker<'a> {
    ...
    pub fn convert_with_semantics(&mut self) -> Result<CSTNode, CompileError> {
        let root_id = self
            .root_id
            .ok_or_else(|| self.error("No root node found"))?;
            ...
            loop {
                ...
                if let Some(field_name) = cursor.field_name() {
                    if let Some(semantic_node) = self.semantic_map.get_mut(&node_id) {
                        semantic_node
                            .fields
                            .insert(field_name.to_string(), child_id);
                    }
                }
    ...
    fn analyze_node_semantics(&mut self, node_id: NodeId) -> Result<(), CompileError> {
        ...
        let (field_mappings, issues, strategy) = match node_type.as_str() {
            ...
            "scatter_assignment" | "local_assign_scatter" | "scatter_assign" => {
                self.analyze_scatter_assignment(node_id)?
            }
            "scatter_pattern" => self.analyze_scatter_pattern(node_id)?,
            "scatter_target" | "scatter_optional" | "scatter_rest" => {
                self.analyze_scatter_item(node_id)?
            }
            ...
            "conditional_operation" | "conditional_expr" => {
                self.analyze_conditional_operation(node_id)?
            }
            ...
            _ => (
                Vec::new(),
                Vec::new(),
                self.default_conversion_strategy(&node_type),
            ),
        };
    ...
    fn analyze_if_statement(
        &mut self,
        node_id: NodeId,
    ) -> Result<(Vec<FieldMapping>, Vec<StructuralIssue>, ConversionStrategy), CompileError> {
    ...
    fn analyze_conditional_operation(
        &mut self,
        _node_id: NodeId,
    ) -> Result<(Vec<FieldMapping>, Vec<StructuralIssue>, ConversionStrategy), CompileError> {
        let strategy = ConversionStrategy::FieldBased {};
    ...
    fn analyze_scatter_pattern(
        ...
        for &child_id in &semantic_node.children {
            let child_type = &self.semantic_map[&child_id].node_type;
            if child_type == "scatter_target"
                || child_type == "scatter_optional"
                || child_type == "scatter_rest"
            {
                field_mappings.push(FieldMapping {
    ...
    fn perform_semantic_conversion(&mut self, node_id: NodeId) -> Result<CSTNode, CompileError> {
        ...
        match semantic_node.node_type.as_str() {
            ...
            "let_statement" | "scatter_assignment" | "local_assign_scatter" | "scatter_assign" => {
                self.convert_semantic_scatter_assignment(node_id)
            }
            "binding_pattern" | "scatter_pattern" => self.convert_semantic_scatter_pattern(node_id),
            "binding_optional" | "scatter_target" | "scatter_optional" | "scatter_rest" => {
                self.convert_semantic_scatter_item(node_id)
            }
            ...
            "conditional_operation" | "conditional_expr" => {
                self.convert_semantic_conditional_operation(node_id)
            }
            ...
            "integer" | "INTEGER" => {
                Ok(self.create_terminal(Rule::integer, &semantic_node.text, span))
            }
    ...
    fn convert_semantic_if_statement(&mut self, node_id: NodeId) -> Result<CSTNode, CompileError> {
        let field_mappings = self.semantic_map[&node_id]
            .semantic_info
            .field_mappings
            .clone();
        let span = self.semantic_map[&node_id].span.clone();
    ...
    fn convert_semantic_method_call(&mut self, node_id: NodeId) -> Result<CSTNode, CompileError> {
        let _conversion_strategy = self.semantic_map[&node_id]
            .semantic_info
            .conversion_strategy
            .clone();
    ...
    fn convert_semantic_scatter_assignment(
        &mut self,
        node_id: NodeId,
    ...
    fn convert_semantic_scatter_pattern(
        &mut self,
        node_id: NodeId,
    ) -> Result<CSTNode, CompileError> {
        let fields = self.semantic_map[&node_id].fields.clone();
    ...
    fn convert_scatter_required(&mut self, node_id: NodeId) -> Result<CSTNode, CompileError> {
        let id_node = self.semantic_map[&node_id]
            .fields
            .get("identifier")
            .copied()
    ...
    fn convert_scatter_optional(&mut self, node_id: NodeId) -> Result<CSTNode, CompileError> {
        ...
        let mut children = vec![self.create_terminal_node("?", span.clone())];
    ...
    fn convert_semantic_system_property(
        &mut self,
        node_id: NodeId,
    ) -> Result<CSTNode, CompileError> {
        let fields = self.semantic_map[&node_id].fields.clone();
    ...
    fn convert_semantic_binary_operation(
        &mut self,
        node_id: NodeId,
    ) -> Result<CSTNode, CompileError> {
        let fields = self.semantic_map[&node_id].fields.clone();
    ...
    fn convert_semantic_unary_operation(
        &mut self,
        node_id: NodeId,
    ) -> Result<CSTNode, CompileError> {
        let fields = self.semantic_map[&node_id].fields.clone();
    ...
    fn convert_semantic_conditional_operation(
        &mut self,
        node_id: NodeId,
    ) -> Result<CSTNode, CompileError> {
        let fields = self.semantic_map[&node_id].fields.clone();
    ...
    fn convert_semantic_property_access(
        &mut self,
        node_id: NodeId,
    ) -> Result<CSTNode, CompileError> {
        let fields = self.semantic_map[&node_id].fields.clone();
    ...
    fn convert_semantic_return_expression(
        &mut self,
        node_id: NodeId,
    ) -> Result<CSTNode, CompileError> {
        let fields = self.semantic_map[&node_id].fields.clone();
