...
// Note: Into<Parse> is automatically provided by implementing From<ParseCst> for Parse
...
mod tests {
    ...
    use crate::{
        ...
        parse_cst::parse_program_cst,
        parse_treesitter::parse_program_with_tree_sitter,
        unparse::unparse,
    };
    ...
    fn compare_parsers(source: &str) {
        ...
        match (pest_result, ts_result) {
            ...
            (Err(pest_err), Err(ts_err)) => {
                ...
                println!("Both parsers failed for '{}': PEST: {:?}, TS: {:?}",
                    source, pest_err, ts_err);
            }
            (Ok(_), Err(ts_err)) => {
                println!("Tree-sitter failed but PEST succeeded for '{}': {:?}",
                    source, ts_err);
            }
            (Err(pest_err), Ok(_)) => {
                println!("PEST failed but tree-sitter succeeded for '{}': {:?}",
                    source, pest_err);
            }
        }
    }
    ...
    fn test_error_cases() {
        ...
        for case in cases {
            ...
            assert!(pest_result.is_err() || ts_result.is_err(),
