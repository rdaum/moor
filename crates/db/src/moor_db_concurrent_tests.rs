...
mod tests {
    ...
    use moor_common::model::{
        ArgSpec, CommitResult, PrepSpec, PropFlag, ValSet, VerbArgsSpec, VerbFlag,
        WorldState, WorldStateError,
    };
    use moor_common::program::ProgramType;
    use moor_common::util::BitEnum;
    use moor_var::{v_int, v_str, Obj, Symbol, NOTHING, SYSTEM_OBJECT};
    use shuttle::{sync::Arc, thread, check_random};
    ...
    fn setup_test_db() -> Arc<MoorDB> {
        ...
        let _root = ws.create_object(&SYSTEM_OBJECT, &NOTHING, &SYSTEM_OBJECT, BitEnum::new()).unwrap();
        
        // Create a few test objects
        for i in 1..=5 {
            let obj = ws.create_object(&SYSTEM_OBJECT, &SYSTEM_OBJECT, &SYSTEM_OBJECT, BitEnum::new()).unwrap();
    ...
    fn test_concurrent_property_access() {
        check_random(|| {
            ...
            let handles: Vec<_> = (0..4).map(|_thread_id| {
                let db = db.clone();
                ...
                thread::spawn(move || {
                    for _i in 0..20 {
                        loop {
                            ...
                            // Read current value
                            let current = match ws.retrieve_property(&SYSTEM_OBJECT, &obj, prop_name) {
                            ...
                            if ws.update_property(&SYSTEM_OBJECT, &obj, prop_name, &v_int(new_value)).is_err() {
            ...
            let final_value = ws.retrieve_property(&SYSTEM_OBJECT, &obj, prop_name).unwrap();
    ...
    fn test_concurrent_verb_lookup_and_cache() {
        check_random(|| {
            ...
            let handles: Vec<_> = (0..8).map(|_thread_id| {
                let db = db.clone();
                ...
                thread::spawn(move || {
                    for iteration in 0..100 {
                        ...
                        if ws.find_method_verb_on(&SYSTEM_OBJECT, &obj, verb_name).is_ok() {
    ...
    fn test_concurrent_object_hierarchy_operations() {
        check_random(|| {
            ...
            // Create a hierarchy: root(0) -> parent(6) -> children(7-10)
            {
                ...
                let parent = ws.create_object(&SYSTEM_OBJECT, &SYSTEM_OBJECT, &SYSTEM_OBJECT, BitEnum::new()).unwrap();
                assert_eq!(parent.id().0, 6);
                
                for i in 7..=10 {
                    let child = ws.create_object(&SYSTEM_OBJECT, &parent, &SYSTEM_OBJECT, BitEnum::new()).unwrap();
    ...
    fn test_concurrent_property_definition_and_access() {
        check_random(|| {
            ...
            let define_handles: Vec<_> = (0..2).map(|thread_id| {
                let db = db.clone();
            ...
            let access_handles: Vec<_> = (0..2).map(|_thread_id| {
                let db = db.clone();
                    ...
                    for _attempt in 0..50 {
                        ...
                        if let Ok(props) = ws.properties(&SYSTEM_OBJECT, &obj) {
                            ...
                            for prop in props.iter() {
                                if let Ok(_value) = ws.retrieve_property(&SYSTEM_OBJECT, &obj, prop.name()) {
    ...
    fn test_concurrent_verb_resolution_cache_stress() {
        check_random(|| {
            ...
            // Create a complex inheritance hierarchy for verb resolution testing
            {
                ...
                let parent1 = ws.create_object(&SYSTEM_OBJECT, &SYSTEM_OBJECT, &SYSTEM_OBJECT, BitEnum::new()).unwrap();
                let parent2 = ws.create_object(&SYSTEM_OBJECT, &parent1, &SYSTEM_OBJECT, BitEnum::new()).unwrap();
                let child = ws.create_object(&SYSTEM_OBJECT, &parent2, &SYSTEM_OBJECT, BitEnum::new()).unwrap();
                ...
                thread::spawn(move || {
                    ...
                    for iteration in 0..200 {
                        ...
                        for verb_name in verbs_to_test {
                            ...
                            let result = ws.find_method_verb_on(&SYSTEM_OBJECT, &child_obj, verb_sym);
            ...
            for verb in ["parent_verb", "override_verb", "child_verb"] {
                let total_found: usize = (0..6)
                    .map(|thread_id| {
                        results.get(&format!("{}_{}", verb, thread_id)).copied().unwrap_or(0)
                    })
                    .sum();
            ...
            let nonexistent_total: usize = (0..6)
                .map(|thread_id| {
                    results.get(&format!("nonexistent_verb_{}", thread_id)).copied().unwrap_or(0)
                })
                .sum();
            assert_eq!(nonexistent_total, 1200, "Nonexistent verb handling count mismatch");
                ...
                thread::spawn(move || {
                    for _iteration in 0..30 {
                        loop {
                            ...
                            let initial_value = ws1.retrieve_property(&SYSTEM_OBJECT, &obj, prop_name)
                            ...
                            let concurrent_value = ws2.retrieve_property(&SYSTEM_OBJECT, &obj, prop_name)
                                .unwrap().as_integer().unwrap();
                            ...
                            if ws1.update_property(&SYSTEM_OBJECT, &obj, prop_name, &v_int(new_value)).is_err() {
                                continue;
                            }
