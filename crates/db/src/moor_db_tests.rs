...
mod tests {
    ...
    /// Test that replicates the exact sequence from test_recycle.moot
    /// This reproduces the issue with object recycling and secondary indexes
    #[test]
    fn test_recycle_moot_sequence() {
        let db = test_db();
        let mut ws = DbWorldState {
            tx: db.start_transaction(),
        };

        // Replicate the exact sequence from test_recycle.moot

        // Create object e: parent=nothing, location=nothing
        let e = ws
            .create_object(&SYSTEM_OBJECT, &NOTHING, &SYSTEM_OBJECT, BitEnum::new())
            .unwrap();
        ws.get_tx_mut()
            .set_object_name(&e, "e".to_string())
            .unwrap();

        // Create object b: parent=e, location=e
        let b = ws
            .create_object(&SYSTEM_OBJECT, &e, &SYSTEM_OBJECT, BitEnum::new())
            .unwrap();
        ws.move_object(&SYSTEM_OBJECT, &b, &e).unwrap();
        ws.get_tx_mut()
            .set_object_name(&b, "b".to_string())
            .unwrap();

        // Create object m: parent=b, location=b
        let m = ws
            .create_object(&SYSTEM_OBJECT, &b, &SYSTEM_OBJECT, BitEnum::new())
            .unwrap();
        ws.move_object(&SYSTEM_OBJECT, &m, &b).unwrap();
        ws.get_tx_mut()
            .set_object_name(&m, "m".to_string())
            .unwrap();

        // Create object n: parent=b, location=b
        let n = ws
            .create_object(&SYSTEM_OBJECT, &b, &SYSTEM_OBJECT, BitEnum::new())
            .unwrap();
        ws.move_object(&SYSTEM_OBJECT, &n, &b).unwrap();
        ws.get_tx_mut()
        ...
        assert_eq!(
            ...
            NOTHING,
            "e.parent should be nothing"
        );
        assert_eq!(
            ws.parent_of(&SYSTEM_OBJECT, &b).unwrap(),
            e,
            "b.parent should be e"
        );
        assert_eq!(
            ws.parent_of(&SYSTEM_OBJECT, &m).unwrap(),
            b,
            "m.parent should be b"
        );
        assert_eq!(
            ws.parent_of(&SYSTEM_OBJECT, &n).unwrap(),
            b,
            "n.parent should be b"
        );

        // Verify children relationships before recycle
        assert_eq!(
            ws.children_of(&SYSTEM_OBJECT, &e).unwrap(),
            ObjSet::from_items(&[b]),
            "e.children should be {{b}}"
        );
        assert_eq!(
            ws.children_of(&SYSTEM_OBJECT, &b).unwrap(),
            ObjSet::from_items(&[m, n]),
            "b.children should be {{m, n}}"
        );
        assert_eq!(
            ws.children_of(&SYSTEM_OBJECT, &m).unwrap(),
            ObjSet::empty(),
            "m.children should be empty"
        );
        assert_eq!(
            ws.children_of(&SYSTEM_OBJECT, &n).unwrap(),
            ObjSet::empty(),
            "n.children should be empty"
        );

        // Verify location relationships before recycle
        assert_eq!(
            ws.location_of(&SYSTEM_OBJECT, &e).unwrap(),
            NOTHING,
            "e.location should be nothing"
        );
        assert_eq!(
            ws.location_of(&SYSTEM_OBJECT, &b).unwrap(),
            e,
            "b.location should be e"
        );
        assert_eq!(
            ws.location_of(&SYSTEM_OBJECT, &m).unwrap(),
            b,
            "m.location should be b"
        );
        assert_eq!(
            ws.location_of(&SYSTEM_OBJECT, &n).unwrap(),
            b,
            "n.location should be b"
        );

        // Verify contents relationships before recycle
        assert_eq!(
            ws.contents_of(&SYSTEM_OBJECT, &e).unwrap(),
            ObjSet::from_items(&[b]),
            "e.contents should be {{b}}"
        );
        assert_eq!(
            ws.contents_of(&SYSTEM_OBJECT, &b).unwrap(),
            ObjSet::from_items(&[m, n]),
            "b.contents should be {{m, n}}"
        );
        assert_eq!(
            ws.contents_of(&SYSTEM_OBJECT, &m).unwrap(),
            ObjSet::empty(),
            "m.contents should be empty"
        );
        assert_eq!(
            ws.contents_of(&SYSTEM_OBJECT, &n).unwrap(),
            ObjSet::empty(),
        ...
        // Parent relationships after recycle
        assert_eq!(
            ws.parent_of(&SYSTEM_OBJECT, &e).unwrap(),
            NOTHING,
            "e.parent should still be nothing"
        );
        assert_eq!(
            ws.parent_of(&SYSTEM_OBJECT, &m).unwrap(),
            e,
            "m.parent should now be e (reparented)"
        );
        assert_eq!(
            ws.parent_of(&SYSTEM_OBJECT, &n).unwrap(),
            e,
            "n.parent should now be e (reparented)"
        );

        // Children relationships after recycle
        assert_eq!(
            ws.children_of(&SYSTEM_OBJECT, &e).unwrap(),
            ObjSet::from_items(&[m, n]),
            "e.children should now be {{m, n}}"
        );
        assert_eq!(
            ws.children_of(&SYSTEM_OBJECT, &m).unwrap(),
            ObjSet::empty(),
            "m.children should still be empty"
        );
        assert_eq!(
            ws.children_of(&SYSTEM_OBJECT, &n).unwrap(),
            ObjSet::empty(),
            "n.children should still be empty"
        );

        // Location relationships after recycle
        assert_eq!(
            ws.location_of(&SYSTEM_OBJECT, &e).unwrap(),
            NOTHING,
            "e.location should still be nothing"
        );
        assert_eq!(
            ws.location_of(&SYSTEM_OBJECT, &m).unwrap(),
            NOTHING,
            "m.location should now be nothing (moved out)"
        );
        assert_eq!(
            ws.location_of(&SYSTEM_OBJECT, &n).unwrap(),
            NOTHING,
            "n.location should now be nothing (moved out)"
        );

        // Contents relationships after recycle
        assert_eq!(
            ws.contents_of(&SYSTEM_OBJECT, &e).unwrap(),
            ObjSet::empty(),
            "e.contents should now be empty"
        );
        assert_eq!(
            ws.contents_of(&SYSTEM_OBJECT, &m).unwrap(),
            ObjSet::empty(),
            "m.contents should still be empty"
        );
        assert_eq!(
            ws.contents_of(&SYSTEM_OBJECT, &n).unwrap(),
            ObjSet::empty(),
    ...
    #[test]
    fn test_recycle_separate_transactions() {
        let db = test_db();

        // Create objects in separate transactions like .moot might do
        let (e, b, m, n) = {
            let mut ws = DbWorldState {
                tx: db.start_transaction(),
            };
            ...
            ws.get_tx_mut()
                .set_object_name(&e, "e".to_string())
                .unwrap();
            let commit_result = Box::new(ws).commit().unwrap();
            assert_eq!(commit_result, moor_common::model::CommitResult::Success);
        ...
        // Verify setup in a new transaction
        {
            let ws = DbWorldState {
                tx: db.start_transaction(),
            };
            ...
            let b_children = ws.children_of(&SYSTEM_OBJECT, &b).unwrap();
            assert!(
                b_children.contains(m) && b_children.contains(n) && b_children.len() == 2,
                "b.children should contain m and n"
            );
            assert_eq!(
                ws.location_of(&SYSTEM_OBJECT, &m).unwrap(),
                b,
            ...
            let b_contents = ws.contents_of(&SYSTEM_OBJECT, &b).unwrap();
            assert!(
                b_contents.contains(m) && b_contents.contains(n) && b_contents.len() == 2,
                "b.contents should contain m and n"
            );
        }

        // Now recycle in a new transaction
        {
            let mut ws = DbWorldState {
                tx: db.start_transaction(),
            };
            ...
            assert_eq!(
                ws.parent_of(&SYSTEM_OBJECT, &m).unwrap(),
                e,
                "m.parent should now be e"
            );
            assert_eq!(
                ws.parent_of(&SYSTEM_OBJECT, &n).unwrap(),
                e,
                "n.parent should now be e"
            );
            assert_eq!(
                ws.location_of(&SYSTEM_OBJECT, &m).unwrap(),
                NOTHING,
                "m.location should now be nothing"
            );
            assert_eq!(
                ws.location_of(&SYSTEM_OBJECT, &n).unwrap(),
                NOTHING,
                "n.location should now be nothing"
            );
        ...
        // Final verification in yet another transaction
        {
            let ws = DbWorldState {
                tx: db.start_transaction(),
            };
            ...
            assert_eq!(
                ws.parent_of(&SYSTEM_OBJECT, &m).unwrap(),
                e,
                "m.parent should be e after commit"
            );
            assert_eq!(
                ws.parent_of(&SYSTEM_OBJECT, &n).unwrap(),
                e,
                "n.parent should be e after commit"
            );
            let e_children = ws.children_of(&SYSTEM_OBJECT, &e).unwrap();
            assert!(
                e_children.contains(m) && e_children.contains(n) && e_children.len() == 2,
                "e.children should contain m and n after commit"
            );
            assert_eq!(
                ws.location_of(&SYSTEM_OBJECT, &m).unwrap(),
                NOTHING,
                "m.location should be nothing after commit"
            );
            assert_eq!(
                ws.location_of(&SYSTEM_OBJECT, &n).unwrap(),
                NOTHING,
                "n.location should be nothing after commit"
            );
            assert_eq!(
                ws.contents_of(&SYSTEM_OBJECT, &e).unwrap(),
                ObjSet::empty(),
    ...
    /// This tests whether secondary indexes work when data is committed and read in fresh transactions
    #[test]
    fn test_secondary_index_cross_transaction_persistence() {
        let db = test_db();
        ...
        let (e, b, m, n) = {
            ...
            // Set locations (this should populate object_location secondary index)
            ws.move_object(&SYSTEM_OBJECT, &b, &e).unwrap();
            ws.move_object(&SYSTEM_OBJECT, &m, &b).unwrap();
            ws.move_object(&SYSTEM_OBJECT, &n, &b).unwrap();
        ...
        // In a FRESH transaction, verify secondary indexes work for contents
        {
            let ws = DbWorldState {
                tx: db.start_transaction(),
            };
            ...
            // Critical test: Do secondary indexes work after commit?
            assert!(
                e_contents.contains(b),
                "e.contents should contain b (secondary index test)"
            );
            assert!(
                b_contents.contains(m) && b_contents.contains(n),
                "b.contents should contain m,n (secondary index test)"
            );
            assert!(
                e_children.contains(b),
                "e.children should contain b (secondary index test)"
            );
            assert!(
                b_children.contains(m) && b_children.contains(n),
                "b.children should contain m,n (secondary index test)"
            );

            assert_eq!(e_contents.len(), 1, "e should have exactly 1 content");
            assert_eq!(b_contents.len(), 2, "b should have exactly 2 contents");
            assert_eq!(e_children.len(), 1, "e should have exactly 1 child");
            assert_eq!(b_children.len(), 2, "b should have exactly 2 children");
        }

        // Now recycle b in another fresh transaction
        {
            let mut ws = DbWorldState {
                tx: db.start_transaction(),
            };
            ...
            // Verify we can still see the relationships
            assert!(
                b_contents_before.contains(m) && b_contents_before.contains(n),
                "Should see b.contents before recycle"
            );
            assert!(
                b_children_before.contains(m) && b_children_before.contains(n),
                "Should see b.children before recycle"
            );
        ...
        // Final verification in yet another fresh transaction
        {
            let ws = DbWorldState {
                tx: db.start_transaction(),
            };

            // These should use secondary indexes to see the post-recycle state
            let e_children_after = ws.children_of(&SYSTEM_OBJECT, &e).unwrap();
            let e_contents_after = ws.contents_of(&SYSTEM_OBJECT, &e).unwrap();
            let m_location = ws.location_of(&SYSTEM_OBJECT, &m).unwrap();
            let n_location = ws.location_of(&SYSTEM_OBJECT, &n).unwrap();
            let m_parent = ws.parent_of(&SYSTEM_OBJECT, &m).unwrap();
            let n_parent = ws.parent_of(&SYSTEM_OBJECT, &n).unwrap();

            // The critical cross-transaction secondary index test:
            assert!(
                e_children_after.contains(m) && e_children_after.contains(n),
                "e.children should contain m,n after cross-transaction recycle"
            );
            assert_eq!(
                e_children_after.len(),
                2,
                "e should have exactly 2 children after recycle"
            );
            assert_eq!(
                e_contents_after.len(),
                0,
                "e should have 0 contents after recycle (objects moved to NOTHING)"
            );
            assert_eq!(m_location, NOTHING, "m should be at NOTHING after recycle");
            assert_eq!(n_location, NOTHING, "n should be at NOTHING after recycle");
            assert_eq!(m_parent, e, "m should be child of e after recycle");
            assert_eq!(n_parent, e, "n should be child of e after recycle");
        }
    }
}
