...
mod tests {
    ...
    fn world_with_test_program_parser(program: &str, parser: &dyn MooParser) -> Box<dyn WorldState> {
    ...
    fn run_test_with_parser(program: &str, parser: &dyn MooParser) -> Result<Var, Box<dyn std::error::Error>> {
    ...
    fn test_program_all_parsers(program: &str, expected_result: Var) {
        ...
        for parser in &parsers {
            ...
            match &result {
                Ok(actual) => assert_eq!(actual, &expected_result, 
                    "Parser '{}' returned wrong result for program: {}", parser.name(), program),
                Err(e) => panic!("Parser '{}' failed for program '{}': {}", parser.name(), program, e),
            }
        ...
        if results.len() > 1 {
            ...
            for (parser_name, result) in &results[1..] {
                match (first_result, result) {
                    (Ok(first), Ok(current)) => assert_eq!(current, first, 
                    ...
                    _ => panic!("Parser '{}' had different success/failure status than '{}' for program: {}", 
                        parser_name, results[0].0, program),
                }
            }
        }
    ...
    fn test_all_parsers_if_else() {
        test_program_all_parsers(
            "if (1 == 1) return 42; else return 24; endif",
            v_int(42)
        );
    }
        ...
        for parser in get_available_parsers() {
            ...
            match result {
                Ok(actual) => assert_eq!(actual, expected, 
                    "Parser '{}' returned wrong result", parser.name()),
