...
fn bf_set_verb_code(bf_args: &mut BfCallState<'_>) -> Result<BfRet, BfErr> {
    ...
    let language = if bf_args.args.len() > 3 {
        match bf_args.args[3].as_symbol() {
            Ok(sym) => {
                ...
                if sym_str == "moo" {
                    ...
                    return Err(BfErr::ErrValue(
                        E_INVARG.msg("language must be 'moo or 'js"),
                    ));
                }
            }
            Err(_) => return Err(BfErr::Code(E_TYPE)),
        }
    } else {
        "moo"
    };
    ...
    let program = match language {
        "js" => {
            // For JavaScript, just store the source directly
