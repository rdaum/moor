...
use crate::{
    ...
    vm::{
        ...
        js::js_execute::validate_javascript,
    },
};
use moor_common::{
    model::{
...
fn bf_verb_code(bf_args: &mut BfCallState<'_>) -> Result<BfRet, BfErr> {
    ...
    match &verb_info.0 {
        ...
        ProgramType::JavaScript(source) => {
            // For JavaScript, split source into lines
...
/// MOO: `list set_verb_code(obj object, str|int verb_desc, list code [, symbol language])`
/// Sets the source code of a verb. Returns empty list on success, or compilation errors.
/// Optional language parameter: 'moo (default) or 'js for JavaScript.
fn bf_set_verb_code(bf_args: &mut BfCallState<'_>) -> Result<BfRet, BfErr> {
    ...
    // Parse optional language parameter (defaults to 'moo)
    let language = if bf_args.args.len() > 3 {
        let Ok(sym) = bf_args.args[3].as_symbol() else {
            return Err(BfErr::Code(E_TYPE));
        };
        sym
    } else {
    ...
    // Create the program based on language
    let program = if language == Symbol::mk("js") {
        // For JavaScript, validate the source before storing
        if let Err(errors) = validate_javascript(&code_string) {
            // Return validation errors like MOO compilation errors
            let error_vars: Vec<Var> = errors.iter().map(|s| v_str(s)).collect();
            return Ok(Ret(v_list(&error_vars)));
        }
        ProgramType::JavaScript(code_string)
    } else if language == Symbol::mk("moo") {
        // For MOO, compile the source
        match compile(code_string.as_str(), bf_args.config.compile_options()) {
            Ok(program) => ProgramType::MooR(program),
            Err(e) => {
...
/// MOO: `symbol verb_language(obj object, str|int verb_desc)`
/// Returns the language of a verb as a symbol: 'moo or 'js
fn bf_verb_language(bf_args: &mut BfCallState<'_>) -> Result<BfRet, BfErr> {
    if bf_args.args.len() != 2 {
        return Err(BfErr::Code(E_ARGS));
    }
    let Some(obj) = bf_args.args[0].as_object() else {
    ...
    // Retrieve the verb program to check its type
    let (program, _) = with_current_transaction(|world_state| {
        world_state.retrieve_verb(&bf_args.task_perms_who(), &obj, verbdef.uuid())
    })
    .map_err(world_state_bf_err)?;

    // Return the language based on the program type
    let language = match &program {
        ProgramType::MooR(_) => Symbol::mk("moo"),
        ProgramType::JavaScript(_) => Symbol::mk("js"),
    };

    Ok(Ret(moor_var::v_sym(language)))
}
...
pub(crate) fn register_bf_verbs(builtins: &mut [Box<BuiltinFunction>]) {
    ...
    builtins[offset_for_builtin("verb_language")] = Box::new(bf_verb_language);
