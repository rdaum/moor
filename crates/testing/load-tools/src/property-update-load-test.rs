...
struct Args {
    ...
    #[arg(
        long,
        help = "Number of properties per object",
        default_value = "10"
    )]
...
fn run_workload(
    ...
    for handle in handles {
        let latencies = handle.join().map_err(|_| eyre::eyre!("Worker thread panicked"))??;
...
fn run_benchmark(database: &TxDB, setup: &TestSetup, args: &Args) -> Result<Vec<Results>, eyre::Error> {
    let mut results = vec![];
...
fn run_swamp_mode(
                ...
                while Instant::now() < stop_time {
                    ...
                    loop {
                        ...
                        if is_read {
                            ...
                            let new_value = v_int(rng.random_range(0i64..1_000_000));
                            tx.set_property(&obj, prop, None, None, Some(new_value)).unwrap();
                        }
    ...
    for handle in handles {
        handle.join().map_err(|_| eyre::eyre!("Worker thread panicked"))?;
    }
