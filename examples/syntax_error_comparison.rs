...
fn test_parser_error_reporting(parser_name: &str, test_case: &ErrorTest) -> ErrorResult {
    ...
    let result = match parser_name {
        ...
        #[cfg(feature = "tree-sitter-parser")]
        "tree-sitter" => {
            compile_with_tree_sitter(&test_case.code, compile_options)
        }
        ...
        if let Some(line_end) = error_msg[line_start + 5..].find(',') {
            if let Ok(line) = error_msg[line_start + 5..line_start + 5 + line_end].parse::<usize>() {
                if let Some(col_start) = error_msg[line_start + 5 + line_end..].find("column ") {
                    ...
                    if let Some(col_end) = error_msg[col_start..].find(|c: char| !c.is_ascii_digit()) {
...
fn evaluate_error_quality(error_msg: &str, test_case: &ErrorTest, location: Option<(usize, usize)>) -> u8 {
    let mut score = 0u8;
    ...
    if let (Some((actual_line, actual_col)), Some((expected_line, expected_col))) = (location, test_case.expected_location) {
    ...
    if error_lower.contains(&expected_lower) {
        score += 2;
    } else if error_lower.contains("syntax") || error_lower.contains("parse") || error_lower.contains("unexpected") {
        score += 1;
    }
...
fn print_error_comparison_matrix(results: &[ErrorResult]) {
    ...
    for result in results {
        tests.entry(result.test_name.clone()).or_insert_with(Vec::new).push(result);
    }
    ...
    for parser in &parsers {
        let parser_results: Vec<_> = results.iter().filter(|r| r.parser_name == *parser).collect();
        
        if !parser_results.is_empty() {
            let detection_rate = parser_results.iter().filter(|r| r.success).count() as f64 / parser_results.len() as f64;
            let avg_quality = parser_results.iter().map(|r| r.error_quality_score as f64).sum::<f64>() / parser_results.len() as f64;
            let location_rate = parser_results.iter().filter(|r| r.error_location.is_some()).count() as f64 / parser_results.len() as f64;
            ...
            println!("  Location Rate: {:.1}% ({}/{})", 
                     location_rate * 100.0,
                     parser_results.iter().filter(|r| r.error_location.is_some()).count(),
                     parser_results.len());
            println!();
        }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Running {} test cases across {} parsers...\n", test_cases.len(), parsers.len());
    ...
    for parser in &parsers {
        ...
        for test_case in &test_cases {
            ...
            if result.success {
                println!("  ✅ {}: Quality {}/10", test_case.name, result.error_quality_score);
            } else {
            ...
            // Write error to file
            writeln!(file, "Test Case: {} ({})", test_case.name, test_case.description)?;
            writeln!(file, "Code:")?;
            writeln!(file, "{}", test_case.code)?;
            writeln!(file, "\nError Output:")?;
            writeln!(file, "{}", result.error_message.as_deref().unwrap_or("No error message"))?;
            writeln!(file, "\nError Location: {}", 
