...
def EvalStart(builder):
    builder.StartObject(3)

def Start(builder):
    EvalStart(builder)

def EvalAddClientToken(builder, clientToken):
...
def AddClientToken(builder, clientToken):
    EvalAddClientToken(builder, clientToken)

def EvalAddAuthToken(builder, authToken):
...
def AddAuthToken(builder, authToken):
    EvalAddAuthToken(builder, authToken)

def EvalAddExpression(builder, expression):
    builder.PrependUOffsetTRelativeSlot(2, flatbuffers.number_types.UOffsetTFlags.py_type(expression), 0)

def AddExpression(builder, expression):
    EvalAddExpression(builder, expression)
