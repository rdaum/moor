...
pub fn parse_object_definitions(source: &str) -> Option<Vec<ObjectDefinition>> {
    ...
    compile_object_definitions(source, &options, &mut context).ok()
}
...
pub async fn check_sync_status(source: &str, client: &mut MoorClient) -> Vec<SyncDiagnosticInfo> {
    let Some(object_defs) = parse_object_definitions(source) else {
        return Vec::new();
    };
