...
pub struct Args {
    ...
    #[clap(
        long,
        help = "List available well-known MOO cores"
    )]
    list_cores: bool,
...
fn list_available_cores() {
    ...
    for core in CORE_REGISTRY {
        ...
        println!("    Compressed: {}", if core.compressed { "Yes" } else { "No" });
        println!();
    }
}
...
async fn download_core(url: &str, output_path: &PathBuf, compressed: bool) -> Result<(), eyre::Report> {
    info!("Downloading MOO core from: {}", url);
    
    let response = reqwest::get(url).await?;
    if !response.status().is_success() {
        return Err(eyre::eyre!("Failed to download: HTTP {}", response.status()));
    }
...
async fn benchmark_parser_on_file(core: &CoreDatabase, parser: &str, textdump_path: &Path) -> BenchmarkResult {
...
async fn run_benchmarks(cores_to_test: Option<String>) -> Result<(), eyre::Report> {
    ...
    info!("Running benchmarks for {} cores and {} parsers", cores_to_benchmark.len(), parsers.len());
    ...
    for core in &cores_to_benchmark {
        ...
        for parser in &parsers {
            ...
            if result.success {
                ...
                error!("❌ {}/{}: {}", core.name, parser, result.error_message.as_ref().unwrap_or(&"Unknown error".to_string()));
            }
...
fn print_benchmark_matrix(results: &[BenchmarkResult]) {
    ...
    let mut cores: std::collections::HashMap<String, Vec<&BenchmarkResult>> = std::collections::HashMap::new();
    
    for result in results {
        cores.entry(result.core_name.clone()).or_default().push(result);
    }
    ...
    for parser in &parsers {
        let parser_results: Vec<_> = results.iter().filter(|r| r.parser_name == *parser && r.success).collect();
        
        if !parser_results.is_empty() {
            let avg_parse = parser_results.iter().map(|r| r.parse_time.as_secs_f64()).sum::<f64>() / parser_results.len() as f64;
            let avg_download = parser_results.iter().map(|r| r.download_time.as_secs_f64()).sum::<f64>() / parser_results.len() as f64;
            let avg_total = parser_results.iter().map(|r| r.total_time.as_secs_f64()).sum::<f64>() / parser_results.len() as f64;
            
            println!("{parser} parser averages: Parse: {avg_parse:.2}s, Download: {avg_download:.2}s, Total: {avg_total:.2}s");
        }
    }
    ...
    if !failed_results.is_empty() {
        ...
        for result in failed_results {
            println!("{}/{}: {}", result.core_name, result.parser_name, 
...
fn run_tests(
    ...
    info!("MOOT test summary: {}/{} tests passed", test_count - failed_tests, test_count);
    
    if failed_tests > 0 {
        error!("{} tests failed", failed_tests);
        return Err(eyre::eyre!("{} out of {} MOOT tests failed", failed_tests, test_count));
    }
...
async fn main() -> Result<(), eyre::Report> {
    ...
    let (_temp_file, src_textdump) = if let Some(core_name) = args.src_core {
        let core = find_core_by_name(&core_name).ok_or_else(|| {
            eyre::eyre!("Unknown core '{}'. Use --list-cores to see available cores.", core_name)
        })?;
