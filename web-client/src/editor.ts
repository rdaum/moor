...
export function createEditor(editor_element) {
    ...
    monaco.languages.setMonarchTokensProvider("moo", MooSyntax as any);
    // Each model gets a unique URI, by incrementing a counter.
    const uri = monaco.Uri.parse("file:///model" + model_counter++);

    const model = monaco.editor.createModel("", "moo", uri);
