...
import { matchRef, ObjectRef, ORefKind } from "./var";
...
/**
 * Format an ObjectRef as a string for display purposes
 */
function formatObjectRef(ref: ObjectRef): string {
    switch (ref.kind) {
        case ORefKind.Oid:
            return `#${ref.oid}`;
        case ORefKind.SysObj:
            return `$${ref.sysobj.join(".")}`;
        case ORefKind.Match:
            return ref.match;
        default:
            return "unknown";
...
function displayHistoricalEvent(
    ...
    const contentType = "text/plain"; // Default content type for historical events

    // Extract the actual message content from the nested structure
    const messageContent = typeof event.message === "object" && event.message !== null
        ? event.message.content || ""
        : event.message || "";

    // Extract timestamp from nested structure
    const timestamp = typeof event.timestamp === "object" && event.timestamp !== null
        ? (event.timestamp.secs_since_epoch * 1000) + Math.floor(event.timestamp.nanos_since_epoch / 1000000)
    ...
    const narrativeEvent: NarrativeEvent = {
        ...
        author: formatObjectRef(event.author),
 ...
 */
export function actionInvoke(context: Context, author: string, verb: string, argument?: any): void {
    console.log(`Invoking ${verb} on ${author}`);

    const mrpcObject = new MoorRemoteObject(matchRef(author), context.authToken!);
...
export const action_invoke = (author: string, verb: string, argument?: any) => {
    // This will be bound to a context when attached to window.module
    return actionInvoke((window as any).module.context, author, verb, argument);
};
...
function handleSystemMessage(context: Context, msg: SystemEvent) {
    ...
    const content = msg.system_message;
    context.systemMessage.show(content, 2);

    // Also append to the narrative window.
    const content_node = div({ class: "system_message_narrative" }, content);
...
function handleTraceback(context: Context, traceback: Traceback) {
    const content = traceback.traceback.join("\n");
    const content_node = div({ class: "traceback_narrative" }, content);
    narrativeAppend(content_node);
}
 ...
 */
const OutputWindow = (_context: Context, _player: State<Player>): HTMLElement => {
